...
async function main() {
  ...
  const SABLEContracts = await dh.deploySABLEContractsHardhat(
      ARBITRARY_ADDRESS, 
      ARBITRARY_ADDRESS,
      ARBITRARY_ADDRESS
    )
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  const USDSAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 USDS
  const BNBColl = toBN(toWei('5')) // borrower wants to lock 5 BNB collateral

  // Call deployed TroveManager contract to read the liquidation reserve and latest borrowing fee
  const liquidationReserve = await troveManager.USDS_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(USDSAmount)
  
  // Total debt of the new trove = USDS amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = USDSAmount.add(expectedFee).add(liquidationReserve)

  // Get the nominal NICR of the new trove
  const _1e20 = toBN(toWei('100'))
  let NICR = BNBColl.mul(_1e20).div(expectedDebt)
  ...
  await borrowerOperations.openTrove(maxFee, USDSAmount, upperHint, lowerHint, { value: BNBColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 BNB
  const USDSRepayment = toBN(toWei('230')) // borrower wants to repay 230 USDS

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(USDSRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, USDSRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(USDSAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedUSDSAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated USDS amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedUSDSAmount,
    firstRedemptionHint,
