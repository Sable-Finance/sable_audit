...
const deployLiquity = async () => {
  ...
  const lusdToken = await USDSToken.new(
  ...
  USDSToken.setAsDeployed(lusdToken)
...
const getAddresses = (contracts) => {
  return {
    ...
    USDSToken: contracts.lusdToken.address,
...
const connectContracts = async (contracts, addresses) => {
  ...
  await contracts.troveManager.setUSDSToken(addresses.USDSToken)
