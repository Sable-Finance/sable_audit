...
USDS_GAS_COMPENSATION = 200.0
MIN_NET_DEBT = 1800.0
MAX_FEE = Wei(1e18)

"""# Ether price (exogenous)

Ether is the collateral for USDS. The ether price $P_t^e$ follows 
...
"""# SABLE price
In the first month, the price of SABLE follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_SABLE) represents SABLE price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_SABLE, so that the expected SABLE price increases from price_SABLE_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_SABLE_initial$ \cdot (1+$ drift_SABLE$)^{720}$

The SABLE price from the second month on is endogenously determined.
"""

#SABLE price & airdrop
price_SABLE_initial = 0.4
price_SABLE = [price_SABLE_initial]
sd_SABLE=0.005
drift_SABLE = 0.0035
supply_SABLE=[0]
SABLE_total_supply=100000000

"""**SABLE Endogenous Price**

The staked SABLE pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{SABLE Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ SABLE_total_supply

Therefore, the SABLE price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{SABLE\_total\_supply}E_t$$

Interpretation: The denominator implies that with more SABLE tokens issued, SABLE price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop SABLE gain.
...
> (collateral in ether, debt in USDS, collateral ratio target, rational inattention, collateral ratio)
...
$P_{t}^{l}$ is the price of USDS.

$N_t^o$ is rounded to an integer.

---

The amount of USDS tokens generated by a new trove is
...
#sensitivity to USDS price & issuance fee
alpha = 0.3
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of SABLE token airdropped to the stability pool providers
- $D_{t}^{s}$ is the total amount of USDS deposited in the Stability Pool (see below)
...
print(" - BNB period 1 -")
print(f"Min BNB price: {min(price_ether[1:period1])}")
print(f"Max BNB price: {max(price_ether[1:period1])}")
for i in range(period1, period2):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether2))
print(" - BNB period 2 -")
print(f"Min BNB price: {min(price_ether[period1:period2])}")
print(f"Max BNB price: {max(price_ether[period1:period2])}")
for i in range(period2, period3):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether3))
print(" - BNB period 3 -")
print(f"Min BNB price: {min(price_ether[period2:period3])}")
print(f"Max BNB price: {max(price_ether[period2:period3])}")
for i in range(period3, period4):
    random.seed(2019375+10000*i)
    shock_ether = random.normalvariate(0, sd_ether)
    price_ether.append(price_ether[i-1] * (1 + shock_ether) * (1 + drift_ether4))
print(" - BNB period 4 -")
print(f"Min BNB price: {min(price_ether[period3:period4])}")
print(f"Max BNB price: {max(price_ether[period3:period4])}")
...
"""SABLE Price - First Month"""

#SABLE price
for i in range(1, month):
    random.seed(2+13*i)
    shock_SABLE = random.normalvariate(0,sd_SABLE)  
    price_SABLE.append(price_SABLE[i-1]*(1+shock_SABLE)*(1+drift_SABLE))
...
def pending_liquidations(contracts, price_ether_current):
    ...
    stability_pool_balance = contracts.stabilityPool.getTotalUSDSDeposits()
    trove = last_trove
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_SABLE_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
    return 32e6 * (F ** (index-1) - F ** index)

def liquidate_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDS, price_SABLE_current, data, index):
    if len(active_accounts) == 0:
        return [0, 0]

    stability_pool_previous = contracts.stabilityPool.getTotalUSDSDeposits() / 1e18
    stability_pool_eth_previous = contracts.stabilityPool.getBNB() / 1e18
    ...
    while pending_liquidations(contracts, price_ether_current):
        try:
            ...
            for i in range(NUM_LIQUIDATIONS):
                ...
                ICR = contracts.troveManager.getCurrentICR(trove, Wei(price_ether_current * 1e18))
                print(f"ICR: {ICR}")
    stability_pool_current = contracts.stabilityPool.getTotalUSDSDeposits() / 1e18
    stability_pool_eth_current = contracts.stabilityPool.getBNB() / 1e18
    ...
    liquidation_gain = ether_liquidated * price_ether_current - debt_liquidated * price_USDS
    airdrop_gain = price_SABLE_current * quantity_SABLE_airdrop(index)

    data['liquidation_gain'][index] = liquidation_gain
    data['airdrop_gain'][index] = airdrop_gain

    return_stability = calculate_stability_return(contracts, price_USDS, data, index)

    return [ether_liquidated, return_stability]

def calculate_stability_return(contracts, price_USDS, data, index):
    stability_pool_previous = contracts.stabilityPool.getTotalUSDSDeposits() / 1e18
    if index == 0:
        ...
        return_stability = (year/index) * \
            (sum(data['liquidation_gain'][0:index]) +
             ...
             ) / (price_USDS * stability_pool_previous)
    else:
...
def close_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDS, index):
    if len(active_accounts) == 0:
        return [0]
    ...
    if index <= 240:
        ...
        number_closetroves = max(0, n_steady * (1+shock_closetroves)) + beta*(1-price_USDS)*n_troves
...
def get_lusd_to_repay(accounts, contracts, active_accounts, inactive_accounts, account, debt):
    ...
    if debt > lusdBalance:
        ...
        # try with active accounts, which are more likely to hold USDS
        ...
        if pending > 0:
            print(f"\n ***Error: not enough USDS to repay! {debt / 1e18} USDS for {account}")
...
def adjust_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, index):
    ...
    for i, working_trove in enumerate(active_accounts):
        ...
        if p >= ratio:
            ...
            if check < -1:
                ...
                if pending == 0:
                    contracts.borrowerOperations.repayUSDS(repay_amount, hints[0], hints[1], { 'from': account })
                ...
                if isNewTCRAboveCCR(contracts, 0, False, withdraw_amount_wei, True, floatToWei(price_ether_current)):
                    contracts.borrowerOperations.withdrawUSDS(MAX_FEE, withdraw_amount_wei, hints[0], hints[1], { 'from': account })
                    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
                    issuance_USDS_adjust = issuance_USDS_adjust + rate_issuance * withdraw_amount
    ...
    return [coll_added_float, issuance_USDS_adjust]
...
def open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention, price_ether_current):
    ...
    debtChange = floatToWei(supply_trove) + USDS_GAS_COMPENSATION
...
def open_troves(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDS, index):
    random.seed(2019*index)
    shock_opentroves = random.normalvariate(0,sd_opentroves)
    n_troves = len(active_accounts)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    coll_added = 0
    issuance_USDS_open = 0

    if index <= 0:
        number_opentroves = initial_open
    elif price_USDS <= 1 + rate_issuance:
        number_opentroves = max(0, n_steady * (1+shock_opentroves))
    else:
        number_opentroves = max(0, n_steady * (1+shock_opentroves)) + \
                        alpha * (price_USDS - rate_issuance - 1) * n_troves
    ...
    for i in range(0, number_opentroves):
        ...
        issuance_USDS_open = issuance_USDS_open + rate_issuance * supply_trove
    ...
    return [coll_added, issuance_USDS_open]
...
def stability_update(accounts, contracts, active_accounts, return_stability, index):
    ...
    if stability_pool > stability_pool_previous:
        ...
        current_deposit = contracts.stabilityPool.getCompoundedUSDSDeposit(accounts[0])
...
"""USDS Price, liquidity pool, and redemption

**Price Determination**

---
With the supply and demand of USDS tokens defined above, the price of USDS at the current period is given by the following equilibrium condition:
> $$S_t = D_t^s + D_t^l = D_t^s + D_{t-1}^l (1+\zeta_t^l)(1+\sigma_t^l) (\frac{P_t^l}{P_{t-1}^l})^\delta$$

where $S$ is the total supply of USDS.

Solving this equation gives that:
> $$P_t^l = P_{t-1}^l (\frac{S_t-D_t^s}{D_{t-1}^l(1+\zeta_t^l)(1+\sigma_t^l)})^{1/\delta}$$
"""

def calculate_price(price_USDS, liquidity_pool, liquidity_pool_next):
  # liquidity_pool = supply - stability_pool
  # liquidity_pool_next = liquidity_pool_previous * drift_liquidity * (1+shock_liquidity)
  price_USDS_current= price_USDS * (liquidity_pool / liquidity_pool_next) ** (1/delta)

  return price_USDS_current

""" **Stabilizers**

There are two stabilizers to attenuate USDS price deviation from its target range.
...
The amount of $Q_t^d(c)$ is expected to bring the USDS price back to $1.1+f_t^i$. This means that
...
If $P_t^l < 1-f_t^r$, a fraction $\chi_t$ of USDS in the liquidity pool is used for redemption
> $$D_t^r = \chi_t D_t^l,$$
...
Note that unlike stabilizer 1, stabilizer 2 has impact of USDS price in
...
def redeem_trove(accounts, contracts, i, price_ether_current):
    ...
    [firstRedemptionHint, partialRedemptionHintNICR, truncatedUSDSamount] = contracts.hintHelpers.getRedemptionHints(lusd_balance, price_ether_current, 70)
    if truncatedUSDSamount == Wei(0):
        return None
    ...
    try:
        ...
        print(f"USDS Token:    {contracts.lusdToken.address}")
        print(f"i: {i}")
        print(f"account: {accounts[i]}")
        print(f"USDS bal: {lusd_balance / 1e18}")
        print(f"truncated: {truncatedUSDSamount / 1e18}")
        ...
        print(f"amount: {truncatedUSDSamount}")
...
def price_stabilizer(accounts, contracts, active_accounts, inactive_accounts, price_ether_current, price_USDS, index):

    stability_pool = contracts.stabilityPool.getTotalUSDSDeposits() / 1e18
    redemption_pool = 0
    redemption_fee = 0
    issuance_USDS_stabilizer = 0
    ...
    #Calculating Price
    price_USDS_current = calculate_price(price_USDS, liquidity_pool, liquidity_pool_next)
    rate_issuance = contracts.troveManager.getBorrowingRateWithDecay() / 1e18
    rate_redemption = contracts.troveManager.getRedemptionRateWithDecay() / 1e18

    #Stabilizer
    #Ceiling Arbitrageurs
    if price_USDS_current > 1.1 + rate_issuance:
        supply_wanted = stability_pool + \
                        liquidity_pool_next * \
                        ((1.1+rate_issuance) / price_USDS)**delta
        ...
        issuance_USDS_stabilizer = rate_issuance * supply_trove
        if open_trove(accounts, contracts, active_accounts, inactive_accounts, supply_trove, quantity_ether, CR_ratio, rational_inattention):
            price_USDS_current = 1.1 + rate_issuance
            liquidity_pool = supply_wanted - stability_pool


    #Floor Arbitrageurs
    if price_USDS_current < 1 - rate_redemption:
        ...
        supply_target = stability_pool + \
                        liquidity_pool_next * \
                        ((1-rate_redemption) / price_USDS)**delta
        ...
        if supply_diff < redemption_ratio * liquidity_pool:
            ...
            price_USDS_current = 1 - rate_redemption
        else:
    ...
    return [price_USDS_current, redemption_pool, redemption_fee, issuance_USDS_stabilizer]

"""# SABLE Market"""

def SABLE_market(index, data):
    #quantity_SABLE = (SABLE_total_supply/3)*(1-0.5**(index/period))
    np.random.seed(2+3*index)
    if index <= month:
        price_SABLE_current = price_SABLE[index-1]
        ...
        price_SABLE_current = discount * PE_ratio * annualized_earning / SABLE_total_supply

    #MC_SABLE_current = price_SABLE_current * quantity_SABLE

    return [price_SABLE_current, annualized_earning]
