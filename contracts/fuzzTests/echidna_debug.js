...
contract('Echidna debugger', async accounts => {
  ...
  before(async () => {
    ...
    usdsToken = await USDSToken.at(await echidnaTester.usdsToken())
  ...
  it.only('USDS balance', async () => {
    await echidnaTester.openTroveExt('0', '0', '4210965169908805439447313562489173090')

    const totalSupply = await usdsToken.totalSupply();
    const gasPoolBalance = await usdsToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getUSDSDebt();
    const defaultPoolBalance = await defaultPool.getUSDSDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalUSDSDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = usdsToken.balanceOf(currentTrove);
