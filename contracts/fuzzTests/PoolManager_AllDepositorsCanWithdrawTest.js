...
contract("PoolManager - random liquidations/deposits, then check all depositors can withdraw", async accounts => {
  ...
  const performLiquidation = async (remainingDefaulters, liquidatedAccountsDict) => {
    ...
    const liquidatedUSDS = (await troveManager.Troves(randomDefaulter))[0]
    const liquidatedBNB = (await troveManager.Troves(randomDefaulter))[1]
    ...
    const USDSinPoolBefore = await stabilityPool.getTotalUSDSDeposits()
    const liquidatedTx = await troveManager.liquidate(randomDefaulter, { from: accounts[0] })
    const USDSinPoolAfter = await stabilityPool.getTotalUSDSDeposits()
    ...
    console.log(`Liquidation. addr: ${th.squeezeAddr(randomDefaulter)} ICR: ${ICRPercent}% coll: ${liquidatedBNB} debt: ${liquidatedUSDS} SP USDS before: ${USDSinPoolBefore} SP USDS after: ${USDSinPoolAfter} tx success: ${liquidatedTx.receipt.status}`)
  }
  ...
  const performSPDeposit = async (depositorAccounts, currentDepositors, currentDepositorsDict) => {
    ...
    const userBalance = (await usdsToken.balanceOf(randomDepositor))
    const maxUSDSDeposit = userBalance.div(toBN(dec(1, 18)))

    const randomUSDSAmount = th.randAmountInWei(1, maxUSDSDeposit)

    const depositTx = await stabilityPool.provideToSP(randomUSDSAmount, ZERO_ADDRESS, { from: randomDepositor })
    ...
    console.log(`SP deposit. addr: ${th.squeezeAddr(randomDepositor)} amount: ${randomUSDSAmount} tx success: ${depositTx.receipt.status} `)
  }
  ...
  const clearAllUndercollateralizedTroves = async (price) => {
    ...
    */
    while(await systemContainsTroveUnder100(price) && await troveManager.checkRecoveryMode()) {
      ...
      await usdsToken.transfer(lowestTrove, lowestTroveDebt, {from: whale})
  ...
  const attemptWithdrawAllDeposits = async (currentDepositors) => {
    ...
    for (depositor of currentDepositors) {
      ...
      const finalDeposit = await stabilityPool.getCompoundedUSDSDeposit(depositor)
      const BNBGain = await stabilityPool.getDepositorBNBGain(depositor)
      ...
      const BNBinSPAfter = (await stabilityPool.getBNB()).toString()
      const USDSinSPAfter = (await stabilityPool.getTotalUSDSDeposits()).toString()
      const USDSBalanceSPAfter = (await usdsToken.balanceOf(stabilityPool.address))
      const depositAfter = await stabilityPool.getCompoundedUSDSDeposit(depositor)
      ...
      console.log(`--Before withdrawal--
                    ...
                    --After withdrawal--
                     ...
                     BNB remaining in SP: ${BNBinSPAfter}
                     SP USDS deposits tracker after: ${USDSinSPAfter}
                     SP USDS balance after: ${USDSBalanceSPAfter}
                     `)
  ...
  describe("Stability Pool Withdrawals", async () => {
    ...
    it("Defaulters' Collateral in range [1, 1e8]. SP Deposits in range [100, 1e10]. BNB:USD = 100", async () => {
      ...
      const defaulterUSDSProportionMin = 91
      const defaulterUSDSProportionMax = 180

      const depositorCollMin = 1
      const depositorCollMax = 100000000
      const depositorUSDSProportionMin = 100
      const depositorUSDSProportionMax = 100
      ...
      // setup:
      // account set L all add coll and withdraw USDS
      await th.openTrove_allAccounts_randomBNB_randomUSDS(defaulterCollMin,
        defaulterCollMax,
        defaulterAccounts,
        contracts,
        defaulterUSDSProportionMin,
        defaulterUSDSProportionMax,
        true)

      // account set S all add coll and withdraw USDS
      await th.openTrove_allAccounts_randomBNB_randomUSDS(depositorCollMin,
        depositorCollMax,
        depositorAccounts,
        contracts,
        depositorUSDSProportionMin,
        depositorUSDSProportionMax,
        true)
      ...
      const totalUSDSDepositsBeforeWithdrawals = await stabilityPool.getTotalUSDSDeposits()
      const totalBNBRewardsBeforeWithdrawals = await stabilityPool.getBNB()

      await attemptWithdrawAllDeposits(currentDepositors)

      const totalUSDSDepositsAfterWithdrawals = await stabilityPool.getTotalUSDSDeposits()
      const totalBNBRewardsAfterWithdrawals = await stabilityPool.getBNB()

      console.log(`Total USDS deposits before any withdrawals: ${totalUSDSDepositsBeforeWithdrawals}`)
      console.log(`Total BNB rewards before any withdrawals: ${totalBNBRewardsBeforeWithdrawals}`)

      console.log(`Remaining USDS deposits after withdrawals: ${totalUSDSDepositsAfterWithdrawals}`)
      console.log(`Remaining BNB rewards after withdrawals: ${totalBNBRewardsAfterWithdrawals}`)
    ...
    it("Defaulters' Collateral in range [1, 10]. SP Deposits in range [1e8, 1e10]. BNB:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
    ...
    it("Defaulters' Collateral in range [1e6, 1e8]. SP Deposits in range [100, 1000]. Every liquidation empties the Pool. BNB:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
    ...
    it("Defaulters' Collateral in range [1e6, 1e8]. SP Deposits in range [1e8 1e10]. BNB:USD = 100", async () => {
      // whale adds coll that holds TCR > 150%
      await borrowerOperations.openTrove(0, 0, whale, whale, { from: whale, value: dec(5, 29) })
