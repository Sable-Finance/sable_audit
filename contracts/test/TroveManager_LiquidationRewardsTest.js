...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  const [
    ...
    A, B, C, D, E,
    whale, defaulter_1, defaulter_2, defaulter_3, defaulter_4, funder] = accounts;
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    // L1: B liquidated
    const txB = await troveManager.liquidate(bob, DEFAULT_PRICE_FEED_DATA)
    ...
    // L2: D Liquidated
    const txD = await troveManager.liquidate(dennis, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(txB.receipt.status)
    ...
    const alice_Coll = ((await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingBNBReward(alice)))
      .toString()
    const carol_Coll = ((await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingBNBReward(carol)))
      .toString()

    /* Expected collateral:
    A: Alice receives 0.995 BNB from L1, and ~3/5*0.995 BNB from L2.
    expect aliceColl = 2 + 0.995 + 2.995/4.995 * 0.995 = 3.5916 BNB

    C: Carol receives ~2/5 BNB from L2
    expect carolColl = 2 + 2/4.995 * 0.995 = 2.398 BNB

    Total coll = 4 + 2 * 0.995 BNB
    */
    ...
    const entireSystemColl = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    assert.equal(entireSystemColl, A_coll.add(C_coll).add(th.applyLiquidationFee(B_coll.add(D_coll))))

    // check USDS gas compensation
    assert.equal((await usdsToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    // L1: C liquidated
    const txC = await troveManager.liquidate(carol, DEFAULT_PRICE_FEED_DATA)
    ...
    // L2: F Liquidated
    const txF = await troveManager.liquidate(freddy, DEFAULT_PRICE_FEED_DATA)
    ...
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingBNBReward(bob)))
      .toString()
    const dennis_Coll = ((await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingBNBReward(dennis)))
      .toString()
    const erin_Coll = ((await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingBNBReward(erin)))
      .toString()

    /* Expected collateral:
    A and B receives 1/2 BNB * 0.995 from L1.
    total Coll: 3

    A, B, receive (2.4975)/8.995 * 0.995 BNB from L2.
    
    D, E receive 2/8.995 * 0.995 BNB from L2.

    expect A, B coll  = 2 +  0.4975 + 0.2763  =  BNB
    expect D, E coll  = 2 + 0.2212  =  BNB
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has BNB from all previously liquidated troves", async () => {
    // A, B  open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: bob } })
    ...
    // L1: A liquidated
    const txA = await troveManager.liquidate(alice, DEFAULT_PRICE_FEED_DATA)
    ...
    // L5: E Liquidated
    const txE = await troveManager.liquidate(erin, DEFAULT_PRICE_FEED_DATA)
    ...
    const freddy_BNBReward = (await troveManager.getPendingBNBReward(freddy)).toString()

    /* Expected collateral:
     A-E should have been liquidated
     trove F should have acquired all BNB in the system: 1 BNB initial coll, and 0.995^5+0.995^4+0.995^3+0.995^2+0.995 from rewards = 5.925 BNB
    */
    ...
    const gainedBNB = th.applyLiquidationFee(
    ...
    assert.isAtMost(th.getDifference(freddy_BNBReward, gainedBNB), 1000)

    const entireSystemColl = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    assert.isAtMost(th.getDifference(entireSystemColl, F_coll.add(gainedBNB)), 1000)

    // check USDS gas compensation
    assert.equal((await usdsToken.balanceOf(owner)).toString(), dec(1000, 18))
  })
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100000, 18), extraParams: { from: E } })
    ...
    const txA = await troveManager.liquidate(A, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(txA.receipt.status)
    ...
    // Bob adds 1 BNB to his trove
    const addedColl1 = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: addedColl1 })

    // Liquidate C
    const txC = await troveManager.liquidate(C, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(txC.receipt.status)
    ...
    await borrowerOperations.addColl(B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: addedColl2 })

    // Liquidate E
    const txE = await troveManager.liquidate(E, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(txE.receipt.status)
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    // Check accumulated BNB gain for each trove
    const B_BNBGain_1 = await troveManager.getPendingBNBReward(B)
    const C_BNBGain_1 = await troveManager.getPendingBNBReward(C)
    const D_BNBGain_1 = await troveManager.getPendingBNBReward(D)
    const E_BNBGain_1 = await troveManager.getPendingBNBReward(E)

    // Check gains are what we'd expect from a distribution proportional to each trove's entire coll
    const B_expectedPendingBNB_1 = A_collRedistribution.mul(B_entireColl_0).div(denominatorColl_1)
    const C_expectedPendingBNB_1 = A_collRedistribution.mul(C_entireColl_0).div(denominatorColl_1)
    const D_expectedPendingBNB_1 = A_collRedistribution.mul(D_entireColl_0).div(denominatorColl_1)
    const E_expectedPendingBNB_1 = A_collRedistribution.mul(E_entireColl_0).div(denominatorColl_1)

    assert.isAtMost(getDifference(B_expectedPendingBNB_1, B_BNBGain_1), 1e8)
    assert.isAtMost(getDifference(C_expectedPendingBNB_1, C_BNBGain_1), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingBNB_1, D_BNBGain_1), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingBNB_1, E_BNBGain_1), 1e8)

    // // Bob adds 1 BNB to his trove
    await borrowerOperations.addColl(B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: dec(1, 'ether') })
    ...
    const B_BNBGain_2 = await troveManager.getPendingBNBReward(B)
    const D_BNBGain_2 = await troveManager.getPendingBNBReward(D)
    const E_BNBGain_2 = await troveManager.getPendingBNBReward(E)

    // Since B topped up, he has no previous pending BNB gain
    const B_expectedPendingBNB_2 = C_collRedistribution.mul(B_entireColl_1).div(denominatorColl_2)

    // D & E's accumulated pending BNB gain includes their previous gain
    const D_expectedPendingBNB_2 = C_collRedistribution.mul(D_entireColl_1).div(denominatorColl_2)
      .add(D_expectedPendingBNB_1)

    const E_expectedPendingBNB_2 = C_collRedistribution.mul(E_entireColl_1).div(denominatorColl_2)
      .add(E_expectedPendingBNB_1)

    assert.isAtMost(getDifference(B_expectedPendingBNB_2, B_BNBGain_2), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingBNB_2, D_BNBGain_2), 1e8)
    assert.isAtMost(getDifference(E_expectedPendingBNB_2, E_BNBGain_2), 1e8)
    ...
    const B_BNBGain_3 = await troveManager.getPendingBNBReward(B)
    const D_BNBGain_3 = await troveManager.getPendingBNBReward(D)

    // Since B topped up, he has no previous pending BNB gain
    const B_expectedPendingBNB_3 = E_collRedistribution.mul(B_entireColl_2).div(denominatorColl_3)

    // D'S accumulated pending BNB gain includes their previous gain
    const D_expectedPendingBNB_3 = E_collRedistribution.mul(D_entireColl_2).div(denominatorColl_3)
      .add(D_expectedPendingBNB_2)

    assert.isAtMost(getDifference(B_expectedPendingBNB_3, B_BNBGain_3), 1e8)
    assert.isAtMost(getDifference(D_expectedPendingBNB_3, D_BNBGain_3), 1e8)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    //Bob adds BNB to his trove
    const addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob, value: addedColl })

    // Alice withdraws USDS
    await borrowerOperations.withdrawUSDS(th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, DEFAULT_PRICE_FEED_DATA, { from: alice })
    ...
    // Expect Bob now holds all Ether and USDSDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 USDS (10 each for gas compensation)
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingBNBReward(bob)))
      .toString()

    const bob_USDSDebt = ((await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingUSDSDebtReward(bob)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_USDSDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    // D opens trove
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: dennis } })

    // Price drops to 100 $/E
    await priceFeed.setPrice(dec(100, 18))

    // Liquidate D
    const txA = await troveManager.liquidate(dennis, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(txA.receipt.status)
    assert.isFalse(await sortedTroves.contains(dennis))

    /* Bob rewards:
     L1: 1/2*0.995 BNB, 55 USDS
     L2: (2.4975/3.995)*0.995 = 0.622 BNB , 110*(2.4975/3.995)= 68.77 USDSDebt

    coll: 3.1195 BNB
    debt: 233.77 USDSDebt

     Alice rewards:
    L1 1/2*0.995 BNB, 55 USDS
    L2 (1.4975/3.995)*0.995 = 0.3730 BNB, 110*(1.4975/3.995) = 41.23 USDSDebt

    coll: 1.8705 BNB
    debt: 146.23 USDSDebt

    totalColl: 4.99 BNB
    totalDebt 380 USDS (includes 50 each for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingBNBReward(bob)))
      .toString()
    ...
    const alice_USDSDebt = ((await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingUSDSDebtReward(alice)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_USDSDebt, expected_B_debt), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_USDSDebt, expected_A_debt), 10000)
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ extraUSDSAmount: dec(110, 18), extraParams: { from: carol, value: _998_Ether } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: dennis, value: dec(1000, 'ether') } })
    ...
    // Expected rewards:  alice: 1 BNB, bob: 1 BNB, carol: 998 BNB
    const alice_BNBReward_1 = await troveManager.getPendingBNBReward(alice)
    const bob_BNBReward_1 = await troveManager.getPendingBNBReward(bob)
    const carol_BNBReward_1 = await troveManager.getPendingBNBReward(carol)

    //Expect 1000 + 1000*0.995 BNB in system now
    const entireSystemColl_1 = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    ...
    th.assertIsApproximatelyEqual(alice_BNBReward_1.toString(), th.applyLiquidationFee(D_coll).mul(A_coll).div(totalColl))
    th.assertIsApproximatelyEqual(bob_BNBReward_1.toString(), th.applyLiquidationFee(D_coll).mul(B_coll).div(totalColl))
    th.assertIsApproximatelyEqual(carol_BNBReward_1.toString(), th.applyLiquidationFee(D_coll).mul(C_coll).div(totalColl))

    //Carol adds 1 BNB to her trove, brings it to 1992.01 total coll
    const C_addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(carol, carol, DEFAULT_PRICE_FEED_DATA, { from: carol, value: dec(1, 'ether') })

    //Expect 1996 BNB in system now
    const entireSystemColl_2 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    th.assertIsApproximatelyEqual(entireSystemColl_2, totalColl.add(th.applyLiquidationFee(D_coll)).add(C_addedColl))

    // E opens with another 1996 BNB
    ...
    /* Expected BNB rewards: 
     Carol = 1992.01/1996 * 1996*0.995 = 1982.05 BNB
     Alice = 1.995/1996 * 1996*0.995 = 1.985025 BNB
     Bob = 1.995/1996 * 1996*0.995 = 1.985025 BNB

    therefore, expected total collateral:

    Carol = 1991.01 + 1991.01 = 3974.06
    Alice = 1.995 + 1.985025 = 3.980025 BNB
    Bob = 1.995 + 1.985025 = 3.980025 BNB
    ...
    */
    ...
    //Expect 3982.02 BNB in system now
    const entireSystemColl_3 = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))

    // check USDS gas compensation
    th.assertIsApproximatelyEqual((await usdsToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // Expected rewards:  alice: 1 BNB, bob: 1 BNB, carol: 998 BNB (*0.995)
    const alice_BNBReward_1 = await troveManager.getPendingBNBReward(alice)
    const bob_BNBReward_1 = await troveManager.getPendingBNBReward(bob)
    const carol_BNBReward_1 = await troveManager.getPendingBNBReward(carol)

    //Expect 1995 BNB in system now
    const entireSystemColl_1 = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    assert.equal(entireSystemColl_1, A_coll.add(B_coll).add(C_coll).add(th.applyLiquidationFee(D_coll)))
    ...
    /* Alice, Bob, Carol each adds 1 BNB to their troves, 
    bringing them to 2.995, 2.995, 1992.01 total coll each. */

    const addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(alice, alice, DEFAULT_PRICE_FEED_DATA, { from: alice, value: addedColl })
    await borrowerOperations.addColl(bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob, value: addedColl })
    await borrowerOperations.addColl(carol, carol, DEFAULT_PRICE_FEED_DATA, { from: carol, value: addedColl })

    //Expect 1998 BNB in system now
    const entireSystemColl_2 = (await activePool.getBNB()).add(await defaultPool.getBNB()).toString()
    th.assertIsApproximatelyEqual(entireSystemColl_2, totalColl.add(th.applyLiquidationFee(D_coll)).add(addedColl.mul(toBN(3))))

    // E opens with another 1998 BNB
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected BNB rewards: 
     Carol = 1992.01/1998 * 1998*0.995 = 1982.04995 BNB
     Alice = 2.995/1998 * 1998*0.995 = 2.980025 BNB
     Bob = 2.995/1998 * 1998*0.995 = 2.980025 BNB

    therefore, expected total collateral:

    Carol = 1992.01 + 1982.04995 = 3974.05995
    Alice = 2.995 + 2.980025 = 5.975025 BNB
    Bob = 2.995 + 2.980025 = 5.975025 BNB
    ...
    */
    ...
    //Expect 3986.01 BNB in system now
    const entireSystemColl_3 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    //Bob withdraws 0.5 BNB from his trove
    const withdrawnColl = toBN(dec(500, 'finney'))
    await borrowerOperations.withdrawColl(withdrawnColl, bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob })
    ...
    // Expect Bob now holds all Ether and USDSDebt in the system: 2.5 Ether and 300 USDS
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 BNB, 55 USDS
     L2: (0.9975/2.495)*0.995 = 0.3978 BNB , 110*(0.9975/2.495)= 43.98 USDSDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 BNB
    debt: (110 + 55 + 43.98 = 208.98 USDSDebt 

     Alice rewards:
    L1 0.4975, 55 USDS
    L2 (1.4975/2.495)*0.995 = 0.5972 BNB, 110*(1.4975/2.495) = 66.022 USDSDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 BNB
    debt: (50 + 55 + 66.022) = 171.022 USDS Debt

    totalColl: 3.49 BNB
    totalDebt 380 USDS (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingBNBReward(bob)))
      .toString()
    ...
    const entireSystemColl = (await activePool.getBNB()).add(await defaultPool.getBNB())
    ...
    const entireSystemDebt = (await activePool.getUSDSDebt()).add(await defaultPool.getUSDSDebt())
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    //Expect 1995 BNB in system now
    const entireSystemColl_1 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    ...
    //Carol wthdraws 1 BNB from her trove, brings it to 1990.01 total coll
    const C_withdrawnColl = toBN(dec(1, 'ether'))
    await borrowerOperations.withdrawColl(C_withdrawnColl, carol, carol, DEFAULT_PRICE_FEED_DATA, { from: carol })

    //Expect 1994 BNB in system now
    const entireSystemColl_2 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    th.assertIsApproximatelyEqual(entireSystemColl_2, totalColl.add(th.applyLiquidationFee(D_coll)).sub(C_withdrawnColl))

    // E opens with another 1994 BNB
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected BNB rewards: 
     Carol = 1990.01/1994 * 1994*0.995 = 1980.05995 BNB
     Alice = 1.995/1994 * 1994*0.995 = 1.985025 BNB
     Bob = 1.995/1994 * 1994*0.995 = 1.985025 BNB
    ...
    */
    ...
    //Expect 3978.03 BNB in system now
    const entireSystemColl_3 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). A, B, C withdraw. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    /* Alice, Bob, Carol each withdraw 0.5 BNB to their troves, 
    bringing them to 1.495, 1.495, 1990.51 total coll each. */
    const withdrawnColl = toBN(dec(500, 'finney'))
    await borrowerOperations.withdrawColl(withdrawnColl, alice, alice, DEFAULT_PRICE_FEED_DATA, { from: alice })
    await borrowerOperations.withdrawColl(withdrawnColl, bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob })
    await borrowerOperations.withdrawColl(withdrawnColl, carol, carol, DEFAULT_PRICE_FEED_DATA, { from: carol })
    ...
    //Expect 1993.5 BNB in system now
    const entireSystemColl_2 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    ...
    // E opens with another 1993.5 BNB
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: erin, value: entireSystemColl_2 } })
    ...
    /* Expected BNB rewards: 
     Carol = 1990.51/1993.5 * 1993.5*0.995 = 1980.55745 BNB
     Alice = 1.495/1993.5 * 1993.5*0.995 = 1.487525 BNB
     Bob = 1.495/1993.5 * 1993.5*0.995 = 1.487525 BNB

    therefore, expected total collateral:

    Carol = 1990.51 + 1980.55745 = 3971.06745
    Alice = 1.495 + 1.487525 = 2.982525 BNB
    Bob = 1.495 + 1.487525 = 2.982525 BNB
    ...
    */
    ...
    //Expect 3977.0325 BNB in system now
    const entireSystemColl_3 = (await activePool.getBNB()).add(await defaultPool.getBNB())
    th.assertIsApproximatelyEqual(entireSystemColl_3, totalCollAfterL1.add(th.applyLiquidationFee(E_coll)))
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(bob), B_pendingRewardsAfterL1), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(carol), C_pendingRewardsAfterL1), 1000000)
    ...
    await borrowerOperations.addColl(bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob, value: B_addedColl })

    //Carol  withdraws 1 BNB from her trove
    const C_withdrawnColl = toBN(dec(1, 'ether'))
    await borrowerOperations.withdrawColl(C_withdrawnColl, carol, carol, DEFAULT_PRICE_FEED_DATA, { from: carol })
    ...
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(carol), C_pendingRewardsAfterL2), 1000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(dennis), D_pendingRewardsAfterL2), 1000000)
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(110, 18), extraParams: { from: freddy } })

    // D tops up
    const D_addedColl = toBN(dec(1, 'ether'))
    await borrowerOperations.addColl(dennis, dennis, DEFAULT_PRICE_FEED_DATA, { from: dennis, value: D_addedColl })
    ...
    const carol_pendingBNBReward = (await troveManager.getPendingBNBReward(carol)).toString()

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString()
    const dennis_pendingBNBReward = (await troveManager.getPendingBNBReward(dennis)).toString()

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString()
    const erin_pendingBNBReward = (await troveManager.getPendingBNBReward(erin)).toString()
    ...
    // Check pending BNB rewards of C, D, E
    assert.isAtMost(th.getDifference(carol_pendingBNBReward, C_collAfterL3.sub(C_collAfterL1)), 1000000)
    assert.isAtMost(th.getDifference(dennis_pendingBNBReward, D_collAfterL3.sub(D_collAfterL2)), 1000000)
    assert.isAtMost(th.getDifference(erin_pendingBNBReward, E_collAfterL3.sub(E_coll)), 1000000)

    // Check systemic collateral
    const activeColl = (await activePool.getBNB()).toString()
    const defaultColl = (await defaultPool.getBNB()).toString()
    ...
    // check USDS gas compensation
    assert.equal((await usdsToken.balanceOf(owner)).toString(), dec(600, 18))
  })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    // D opens trove: 0.035 BNB
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraUSDSAmount: dec(100, 18), extraParams: { from: dennis, value: toBN(dec(35, 15)) } })

    // Bob adds 11.33909 BNB to his trove
    const B_addedColl = toBN('11339090000000000000')
    await borrowerOperations.addColl(bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob, value: B_addedColl })

    // Carol withdraws 15 BNB from her trove
    ...
    const D_pendingRewardsAfterL2 = D_coll.mul(th.applyLiquidationFee(B_collAfterL1)).div(C_collAfterL1.add(D_coll))
    const C_collAfterL2 = C_collAfterL1.add(C_pendingRewardsAfterL2)
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(carol), C_pendingRewardsAfterL2), 10000000)
    assert.isAtMost(th.getDifference(await troveManager.getPendingBNBReward(dennis), D_pendingRewardsAfterL2), 10000000)
    ...
    F: 0.0007 BNB
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraUSDSAmount: dec(100, 18), extraParams: { from: erin, value: toBN(dec(1, 22)) } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraUSDSAmount: dec(100, 18), extraParams: { from: freddy, value: toBN('700000000000000') } })
