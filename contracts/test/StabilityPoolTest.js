...
contract("StabilityPool", async accounts => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    beforeEach(async () => {
      ...
      await lqtyToken.transfer(communityIssuance.address, toBN((1e19).toString()), {
        from: vaultAddress
      });
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      await troveManager.liquidate(defaulter_1, DEFAULT_PRICE_FEED_DATA, { from: owner }); // 180 LUSD closed
      await troveManager.liquidate(defaulter_2, DEFAULT_PRICE_FEED_DATA, { from: owner }); // 180 LUSD closed
    ...
    it("provideToSP(), new eligible deposit: tagged front end's stake increases", async () => {
      ...
      const F1_Diff = F1_Stake_After.totalDeposits.sub(F1_Stake_Before.totalDeposits);
      const F2_Diff = F2_Stake_After.totalDeposits.sub(F2_Stake_Before.totalDeposits);
      const F3_Diff = F3_Stake_After.totalDeposits.sub(F3_Stake_Before.totalDeposits);
    ...
    it("provideToSP(), topup: tagged front end's stake increases", async () => {
      ...
      assert.isTrue(F1_Stake_After.totalDeposits.gt(F1_Stake_Before.totalDeposits));
      assert.isTrue(F2_Stake_After.totalDeposits.gt(F2_Stake_Before.totalDeposits));
      assert.isTrue(F3_Stake_After.totalDeposits.gt(F3_Stake_Before.totalDeposits));
    });
    ...
    it("withdrawFromSP(): reverts when user has no active deposit", async () => {
      ...
      const txAlice = await stabilityPool.withdrawFromSP(dec(100, 18), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      assert.isTrue(txAlice.receipt.status);

      try {
        const txBob = await stabilityPool.withdrawFromSP(dec(100, 18), DEFAULT_PRICE_FEED_DATA, {
          from: bob
        });
    ...
    it("withdrawFromSP(): reverts when amount > 0 and system has an undercollateralized trove", async () => {
      ...
      await th.assertRevert(
        stabilityPool.withdrawFromSP(dec(100, 18), DEFAULT_PRICE_FEED_DATA, { from: alice })
      );
    });
    ...
    it("withdrawFromSP(): succeeds when amount is 0 and system has an undercollateralized trove", async () => {
      ...
      const tx = await stabilityPool.withdrawFromSP(0, DEFAULT_PRICE_FEED_DATA, {
        from: A,
        gasPrice: GAS_PRICE
      });
    ...
    it("withdrawFromSP(): withdrawing 0 ETH Gain does not alter the caller's ETH balance, their trove collateral, or the ETH  in the Stability Pool", async () => {
      ...
      await stabilityPool.withdrawFromSP(dec(100, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis,
        gasPrice: GAS_PRICE
      });
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      await stabilityPool.withdrawFromSP(bob_Deposit_Before.add(toBN(1)), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      ...
      await stabilityPool.withdrawFromSP("2309842309000000000000000000", DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain during Recovery Mode", async () => {
      ...
      // Price rises
      await priceFeed.setPrice(dec(230, 18));
      ...
      const B_GAS_Deposit = th.gasUsed(
        await stabilityPool.withdrawFromSP(dec(5000, 18), DEFAULT_PRICE_FEED_DATA, {
          from: bob,
          gasPrice: GAS_PRICE
        })
      );
      const C_GAS_Deposit = th.gasUsed(
        await stabilityPool.withdrawFromSP(dec(3000, 18), DEFAULT_PRICE_FEED_DATA, {
          from: carol,
          gasPrice: GAS_PRICE
        })
      );
    ...
    it("withdrawFromSP(), partial withdrawal: tagged front end's stake decreases", async () => {
      ...
      assert.isTrue(F1_Stake_After.totalDeposits.lt(F1_Stake_Before.totalDeposits));
      assert.isTrue(F2_Stake_After.totalDeposits.lt(F2_Stake_Before.totalDeposits));
      assert.isTrue(F3_Stake_After.totalDeposits.lt(F3_Stake_Before.totalDeposits));
    });
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      const withdrawalPromise_A = stabilityPool.withdrawFromSP(
        dec(10000, 18),
        DEFAULT_PRICE_FEED_DATA,
        { from: A }
      );
      ...
      const withdrawalPromise_C = stabilityPool.withdrawFromSP(
        dec(10000, 18),
        DEFAULT_PRICE_FEED_DATA,
        { from: C }
      );
    ...
    it("withdrawETHGainToTrove(): reverts when user has no active deposit", async () => {
      ...
      const txPromise_B = stabilityPool.withdrawETHGainToTrove(bob, bob, DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      await th.assertRevert(txPromise_B);
    });
    ...
    it("withdrawETHGainToTrove(): Applies LUSDLoss to user's deposit, and redirects ETH reward to user's Trove", async () => {
      ...
      await stabilityPool.withdrawETHGainToTrove(alice, alice, DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      ...
      const Trove_ETH_Increase = aliceTrove_ETH_After.sub(aliceTrove_ETH_Before);

      assert.isAtMost(
        th.getDifference(Trove_ETH_Increase, ETHGain_A),
        1000
      );
    });
    ...
    it("withdrawETHGainToTrove(): reverts if it would leave trove with ICR < MCR", async () => {
      ...
      await assertRevert(
        stabilityPool.withdrawETHGainToTrove(alice, alice,DEFAULT_PRICE_FEED_DATA,  { from: alice }),
    ...
    it("withdrawETHGainToTrove(): All depositors are able to withdraw their ETH gain from the SP to their Trove", async () => {
      ...
      // All depositors attempt to withdraw
      const tx1 = await stabilityPool.withdrawETHGainToTrove(alice, alice, DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      assert.isTrue(tx1.receipt.status);
      const tx2 = await stabilityPool.withdrawETHGainToTrove(bob, bob, DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      assert.isTrue(tx1.receipt.status);
      const tx3 = await stabilityPool.withdrawETHGainToTrove(carol, carol, DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
      assert.isTrue(tx1.receipt.status);
      ...
      assert.isTrue(tx1.receipt.status);
      const tx5 = await stabilityPool.withdrawETHGainToTrove(erin, erin, DEFAULT_PRICE_FEED_DATA, {
        from: erin
      });
      assert.isTrue(tx1.receipt.status);
      const tx6 = await stabilityPool.withdrawETHGainToTrove(flyn, flyn, DEFAULT_PRICE_FEED_DATA, {
        from: flyn
      });
      assert.isTrue(tx1.receipt.status);
    });
    ...
    it("withdrawETHGainToTrove(): All depositors withdraw, each withdraw their correct ETH gain", async () => {
      ...
      */
      ...
      await stabilityPool.withdrawETHGainToTrove(bob, bob, DEFAULT_PRICE_FEED_DATA, { from: bob });
      ...
      await stabilityPool.withdrawETHGainToTrove(carol, carol, DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
      ...
      await stabilityPool.withdrawETHGainToTrove(dennis, dennis, DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
      ...
      await stabilityPool.withdrawETHGainToTrove(erin, erin, DEFAULT_PRICE_FEED_DATA, {
        from: erin
      });
      ...
      await stabilityPool.withdrawETHGainToTrove(flyn, flyn, DEFAULT_PRICE_FEED_DATA, {
        from: flyn
      });
    ...
    it("withdrawETHGainToTrove(): caller can withdraw full deposit and ETH gain to their trove during Recovery Mode", async () => {
      ...
      const alice_expectedCollateral = alice_Collateral_Before.add(alice_ETHGain_Before)
      const bob_expectedColalteral = bob_Collateral_Before.add(bob_ETHGain_Before)
      const carol_expectedCollateral = carol_Collateral_Before.add(carol_ETHGain_Before)
      ...
      assert.isAtMost(th.getDifference(alice_expectedCollateral, alice_Collateral_After), 1000);
      assert.isAtMost(th.getDifference(bob_expectedColalteral, bob_Collateral_After), 1000);
      assert.isAtMost(th.getDifference(carol_expectedCollateral, carol_Collateral_After), 1000);
    ...
    it("withdrawETHGainToTrove(): reverts if user has no trove", async () => {
      ...
      await th.assertRevert(
        stabilityPool.withdrawETHGainToTrove(dennis, dennis, DEFAULT_PRICE_FEED_DATA, {
          from: dennis
        }),
    ...
    it("withdrawETHGainToTrove(), partial withdrawal: doesn't change the front end tag", async () => {
      ...
      await stabilityPool.withdrawETHGainToTrove(A, A, DEFAULT_PRICE_FEED_DATA, { from: A });
      await stabilityPool.withdrawETHGainToTrove(B, B, DEFAULT_PRICE_FEED_DATA, { from: B });
      await stabilityPool.withdrawETHGainToTrove(C, C, DEFAULT_PRICE_FEED_DATA, { from: C });
    ...
    it("withdrawETHGainToTrove(): reverts when depositor has no ETH gain", async () => {
      ...
      const txPromise_A = stabilityPool.withdrawETHGainToTrove(A, A, DEFAULT_PRICE_FEED_DATA, {
        from: A
      });
      const txPromise_B = stabilityPool.withdrawETHGainToTrove(B, B, DEFAULT_PRICE_FEED_DATA, {
        from: B
      });
      const txPromise_C = stabilityPool.withdrawETHGainToTrove(C, C, DEFAULT_PRICE_FEED_DATA, {
        from: C
      });
      const txPromise_D = stabilityPool.withdrawETHGainToTrove(D, D, DEFAULT_PRICE_FEED_DATA, {
        from: D
      });
