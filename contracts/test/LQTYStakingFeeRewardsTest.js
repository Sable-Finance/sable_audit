...
contract('LQTYStaking revenue share tests', async accounts => {
  ...
  it('stake(): reverts if amount is zero', async () => {
    // vaultAddress transfers LQTY to staker A
    await lqtyToken.transfer(A, dec(100, 18), {from: vaultAddress})
  ...
  it("ETH fee per LQTY staked increases when a redemption fee is triggered and totalStakes > 0", async () => {
    ...
    // vaultAddress transfers LQTY to staker A
    await lqtyToken.transfer(A, dec(100, 18), {from: vaultAddress, gasPrice: GAS_PRICE})
  ...
  it("LUSD fee per LQTY staked increases when a redemption fee is triggered and totalStakes > 0", async () => {
    ...
    // D draws debt
    const tx = await borrowerOperations.withdrawLUSD(th._100pct, dec(27, 18), D, D, DEFAULT_PRICE_FEED_DATA, {from: D})
  ...
  it("LQTY Staking: A single staker earns all ETH and LQTY fees that occur", async () => {
    ...
    // D draws debt
    const borrowingTx_1 = await borrowerOperations.withdrawLUSD(th._100pct, dec(104, 18), D, D, DEFAULT_PRICE_FEED_DATA, {from: D})
    ...
    // B draws debt
    const borrowingTx_2 = await borrowerOperations.withdrawLUSD(th._100pct, dec(17, 18), B, B, DEFAULT_PRICE_FEED_DATA, {from: B})
  ...
  it("LQTY Staking: Multiple stakers earn the correct share of all ETH and LQTY fees, based on their stake size", async () => {
    ...
    // vaultAddress transfers LQTY to staker A, B, C
    await lqtyToken.transfer(A, dec(100, 18), {from: vaultAddress})
    await lqtyToken.transfer(B, dec(200, 18), {from: vaultAddress})
    await lqtyToken.transfer(C, dec(300, 18), {from: vaultAddress})
    ...
    // F draws debt
    const borrowingTx_1 = await borrowerOperations.withdrawLUSD(th._100pct, dec(104, 18), F, F, DEFAULT_PRICE_FEED_DATA, {from: F})
    const emittedLUSDFee_1 = toBN(th.getLUSDFeeFromLUSDBorrowingEvent(borrowingTx_1))
    assert.isTrue(emittedLUSDFee_1.gt(toBN('0')))

    // G draws debt
    const borrowingTx_2 = await borrowerOperations.withdrawLUSD(th._100pct, dec(17, 18), G, G, DEFAULT_PRICE_FEED_DATA, {from: G})
    const emittedLUSDFee_2 = toBN(th.getLUSDFeeFromLUSDBorrowingEvent(borrowingTx_2))
    assert.isTrue(emittedLUSDFee_2.gt(toBN('0')))

    // D obtains LQTY from owner and makes a stake
    await lqtyToken.transfer(D, dec(50, 18), {from: vaultAddress})
    ...
    assert.equal(await lqtyStaking.totalLQTYStaked(), dec(650, 18))
     ...
     // G draws debt
    const borrowingTx_3 = await borrowerOperations.withdrawLUSD(th._100pct, dec(17, 18), G, G, DEFAULT_PRICE_FEED_DATA, {from: G})
  ...
  it("unstake(): reverts if caller has ETH gains and can't receive ETH",  async () => {
    ...
    // vaultAddress transfers LQTY to staker A and the non-payable proxy
    await lqtyToken.transfer(A, dec(100, 18), {from: vaultAddress})
    await lqtyToken.transfer(nonPayable.address, dec(100, 18), {from: vaultAddress})
