...
contract('TroveManager', async accounts => {
  ...
  const [
    ...
    const [vaultAddress, lqtyStakingAddress, multisig] = accounts.slice(997, 1000)
  ...
  beforeEach(async () => {
    ...
    const LQTYContracts = await deploymentHelper.deployLQTYContracts(vaultAddress, MINT_AMOUNT)
  ...
  it("liquidate(): decreases ActivePool ETH and LUSDDebt by correct amounts", async () => {
    ...
    assert.equal(Number(activePool_ETH_Before), Number(A_collateral.add(B_collateral)))
    ...
    assert.equal(Number(activePool_ETH_After), Number(A_collateral))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    await borrowerOperations.repayLUSD(dec(1, 18), D, D, DEFAULT_PRICE_FEED_DATA, {from: D})
    await borrowerOperations.repayLUSD(dec(1, 18), E, E, DEFAULT_PRICE_FEED_DATA, {from: E})
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(10000, 18)), A, A, DEFAULT_PRICE_FEED_DATA, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(20000, 18)), B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(30000, 18)), C, C, DEFAULT_PRICE_FEED_DATA, { from: C, value: dec(1000, 'ether') })
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(6000, 18)), A, A, DEFAULT_PRICE_FEED_DATA, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(20000, 18)), B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveLUSDAmount(dec(30000, 18)), C, C, DEFAULT_PRICE_FEED_DATA, { from: C, value: dec(1000, 'ether') })
    ...
    // LUSD redemption is 54500 LUSD
    const LUSDRedemption = dec(54500, 18)
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraLUSDAmount: dec(15, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    ...
    const slightlyMoreThanFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn, DEFAULT_ORACLE_RATE))
    ...
    // Attempt with maxFee = 5.5%
    const exactSameFee = (await troveManager.getRedemptionFeeWithDecay(ETHDrawn, DEFAULT_ORACLE_RATE))
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed LUSD (assuming 1 LUSD has value of $1)", async () => {
    ...
    assert.equal(activeETH_0.toString(), totalColl.toString());
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    ETHfee = (0.005 + 0.0025 + (1/2) *( 9/260)) * ETHDrawn = 0.0011163461538461537 ETH
    ETHRemainder = 0.045 - 0.0011163461538461537... = 0.04388365384615384
    */
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const AdjustTroveParam = {
        collWithdrawal: 0,
        LUSDChange: lusdAmount.toString(),
        isDebtIncrease: true,
        upperHint: alice,
        lowerHint: alice,
      ...
      await borrowerOperations.adjustTrove(AdjustTroveParam, DEFAULT_PRICE_FEED_DATA, { from: alice, value: lusdAmount.mul(mv._1e18BN).div(price) })
    }
