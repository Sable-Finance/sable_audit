...
const assertInvalidOpcode = th.assertInvalidOpcode
...
contract('TroveManager', async accounts => {
  ...
  const [
    ...
    const [vaultAddress, sableStakingAddress, multisig] = accounts.slice(997, 1000)
  ...
  const getOpenTroveTotalDebt = async (usdsAmount) => th.getOpenTroveTotalDebt(contracts, usdsAmount)
  ...
  const withdrawUSDS = async (params) => th.withdrawUSDS(contracts, params)
  ...
  it('liquidate(): closes a Trove that has ICR < MCR', async () => {
    ...
    assert.equal(Number(ICR_Before), Number(dec(4, 18)))

    const MCR = (await systemState.getMCR()).toString()
    assert.equal(MCR.toString(), '1100000000000000000')

    // Alice increases debt to 180 USDS, lowering her ICR to 1.11
    const A_USDSWithdrawal = await getNetBorrowingAmount(dec(130, 18))

    const targetICR = toBN('1111111111111111111')
    await withdrawUSDS({ ICR: targetICR, extraParams: { from: alice } })
    ...
    // price drops to 1BNB:100USDS, reducing Alice's ICR below MCR
    ...
    // close Trove
    await troveManager.liquidate(alice, DEFAULT_PRICE_FEED_DATA, { from: owner });
  ...
  it("liquidate(): decreases ActivePool BNB and USDSDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool BNB and USDS debt before
    const activePool_BNB_Before = (await activePool.getBNB()).toString()
    const activePool_RawEther_Before = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_USDSDebt_Before = (await activePool.getUSDSDebt()).toString()

    assert.equal(Number(activePool_BNB_Before), Number(A_collateral.add(B_collateral)))
    assert.equal(Number(activePool_RawEther_Before), Number(A_collateral.add(B_collateral)))
    th.assertIsApproximatelyEqual(activePool_USDSDebt_Before, A_totalDebt.add(B_totalDebt))

    // price drops to 1BNB:100USDS, reducing Bob's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and USDS, 
    leaving Alice’s ether and USDS debt in the ActivePool. */
    await troveManager.liquidate(bob, DEFAULT_PRICE_FEED_DATA, { from: owner });

    // check ActivePool BNB and USDS debt 
    const activePool_BNB_After = (await activePool.getBNB()).toString()
    const activePool_RawEther_After = (await web3.eth.getBalance(activePool.address)).toString()
    const activePool_USDSDebt_After = (await activePool.getUSDSDebt()).toString()

    assert.equal(Number(activePool_BNB_After), Number(A_collateral))
    assert.equal(Number(activePool_RawEther_After), Number(A_collateral))
    th.assertIsApproximatelyEqual(activePool_USDSDebt_After, A_totalDebt)
  })

  it("liquidate(): increases DefaultPool BNB and USDS debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(21, 17)), extraParams: { from: bob } })
    ...
    // check DefaultPool BNB and USDS debt before
    const defaultPool_BNB_Before = (await defaultPool.getBNB())
    const defaultPool_RawEther_Before = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_USDSDebt_Before = (await defaultPool.getUSDSDebt()).toString()

    assert.equal(defaultPool_BNB_Before, '0')
    assert.equal(defaultPool_RawEther_Before, '0')
    assert.equal(defaultPool_USDSDebt_Before, '0')
    ...
    // check after
    const defaultPool_BNB_After = (await defaultPool.getBNB()).toString()
    const defaultPool_RawEther_After = (await web3.eth.getBalance(defaultPool.address)).toString()
    const defaultPool_USDSDebt_After = (await defaultPool.getUSDSDebt()).toString()

    const defaultPool_BNB = th.applyLiquidationFee(B_collateral)
    assert.equal(defaultPool_BNB_After, defaultPool_BNB)
    assert.equal(defaultPool_RawEther_After, defaultPool_BNB)
    th.assertIsApproximatelyEqual(defaultPool_USDSDebt_After, B_totalDebt)
  })
  ...
  it("liquidate(): removes the Trove's stake from the total stakes", async () => {
    ...
    assert.equal(Number(totalStakes_Before), Number(A_collateral.add(B_collateral)))
    ...
    assert.equal(Number(totalStakes_After), Number(A_collateral))
  })
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and USDS should be added to the DefaultPool.
    await troveManager.liquidate(bob, DEFAULT_PRICE_FEED_DATA, { from: owner });

    /* check snapshots after. Total stakes should be equal to the  remaining stake then the system: 
    10 ether, Alice's stake.
     
    Total collateral should be equal to Alice's collateral plus her pending BNB reward (Bob’s collaterale*0.995 ether), earned
    ...
    assert.equal(Number(totalStakesSnapshot_After), Number(A_collateral))
    assert.equal(Number(totalCollateralSnapshot_After), Number(A_collateral.add(th.applyLiquidationFee(B_collateral))))
  })

  it("liquidate(): updates the L_BNB and L_USDSDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    ...
    // price drops to 1BNB:100USDS, reducing Carols's ICR below MCR
    await priceFeed.setPrice('100000000000000000000');
    ...
    // Carol's ether*0.995 and USDS should be added to the DefaultPool.
    const L_BNB_AfterCarolLiquidated = await troveManager.L_BNB()
    const L_USDSDebt_AfterCarolLiquidated = await troveManager.L_USDSDebt()

    const L_BNB_expected_1 = th.applyLiquidationFee(C_collateral).mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    const L_USDSDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral))
    assert.isAtMost(th.getDifference(L_BNB_AfterCarolLiquidated, L_BNB_expected_1), 100)
    assert.isAtMost(th.getDifference(L_USDSDebt_AfterCarolLiquidated, L_USDSDebt_expected_1), 100)

    // Bob now withdraws USDS, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawUSDS({ ICR: toBN(dec(111, 16)), extraParams: { from: bob } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    // price drops to 1BNB:50USDS, reducing Bob's ICR below MCR
   ...
   L_BNB = (0.995 / 20) + (10.4975*0.995  / 10) = 1.09425125 BNB
   L_USDSDebt = (180 / 20) + (890 / 10) = 98 USDS */
    const L_BNB_AfterBobLiquidated = await troveManager.L_BNB()
    const L_USDSDebt_AfterBobLiquidated = await troveManager.L_USDSDebt()

    const L_BNB_expected_2 = L_BNB_expected_1.add(th.applyLiquidationFee(B_collateral.add(B_collateral.mul(L_BNB_expected_1).div(mv._1e18BN))).mul(mv._1e18BN).div(A_collateral))
    const L_USDSDebt_expected_2 = L_USDSDebt_expected_1.add(B_totalDebt.add(B_increasedTotalDebt).add(B_collateral.mul(L_USDSDebt_expected_1).div(mv._1e18BN)).mul(mv._1e18BN).div(A_collateral))
    assert.isAtMost(th.getDifference(L_BNB_AfterBobLiquidated, L_BNB_expected_2), 100)
    assert.isAtMost(th.getDifference(L_USDSDebt_AfterBobLiquidated, L_USDSDebt_expected_2), 100)
  })

  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    await openTrove({ ICR: toBN(dec(200, 18)), extraParams: { from: bob, value: dec(100, 'ether') } })

    // Alice creates a single trove with 0.7 BNB and a debt of 70 USDS, and provides 10 USDS to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })

    // Alice proves 10 USDS to SP
    await stabilityPool.provideToSP(dec(10, 18), ZERO_ADDRESS, { from: alice })

    // Set BNB:USD price to 105
  ...
  it("liquidate(): reverts if trove is non-existent", async () => {
    ...
    try {
      const txCarol = await troveManager.liquidate(carol, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidate(): reverts if trove has been closed", async () => {
    ...
    const txCarol_L1 = await troveManager.liquidate(carol, DEFAULT_PRICE_FEED_DATA)
    ...
    try {
      const txCarol_L2 = await troveManager.liquidate(carol, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides USDS to SP
    const spDeposit = toBN(dec(100, 24))
    await openTrove({ ICR: toBN(dec(4, 18)), extraUSDSAmount: spDeposit, extraParams: { from: whale } })
    ...
    await troveManager.liquidate(defaulter_4, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidate(): a pure redistribution reduces the TCR only as a result of compensation", async () => {
    ...
    const liquidationTx_1 = await troveManager.liquidate(defaulter_1, DEFAULT_PRICE_FEED_DATA)
    ...
    const liquidationTx_2 = await troveManager.liquidate(defaulter_2, DEFAULT_PRICE_FEED_DATA)
    ...
    const liquidationTx_3 = await troveManager.liquidate(defaulter_3, DEFAULT_PRICE_FEED_DATA)
    ...
    const liquidationTx_4 = await troveManager.liquidate(defaulter_4, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidate(): does not affect the SP deposit or BNB gain when called on an SP depositor's address that has no trove", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraUSDSAmount: spDeposit, extraParams: { from: bob } })
    const { C_totalDebt, C_collateral } = await openTrove({ ICR: toBN(dec(218, 16)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Bob sends tokens to Dennis, who has no trove
    await usdsToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides USDS to SP
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: dennis })

    // Carol gets liquidated
    await priceFeed.setPrice(dec(100, 18))
    const liquidationTX_C = await troveManager.liquidate(carol, DEFAULT_PRICE_FEED_DATA)
    const [liquidatedDebt, liquidatedColl, gasComp] = th.getEmittedLiquidationValues(liquidationTX_C)

    assert.isFalse(await sortedTroves.contains(carol))
    // Check Dennis' SP deposit has absorbed Carol's debt, and he has received her liquidated BNB
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedUSDSDeposit(dennis)).toString()
    const dennis_BNBGain_Before = (await stabilityPool.getDepositorBNBGain(dennis)).toString()
    assert.isAtMost(th.getDifference(dennis_Deposit_Before, spDeposit.sub(liquidatedDebt)), 1000000)
    assert.isAtMost(th.getDifference(dennis_BNBGain_Before, liquidatedColl), 1000)
    ...
    try {
      const txDennis = await troveManager.liquidate(dennis, DEFAULT_PRICE_FEED_DATA)
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedUSDSDeposit(dennis)).toString()
    const dennis_BNBGain_After = (await stabilityPool.getDepositorBNBGain(dennis)).toString()
    assert.equal(dennis_Deposit_Before, dennis_Deposit_After)
    assert.equal(dennis_BNBGain_Before, dennis_BNBGain_After)
  })

  it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or BNB gain", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(3, 18)), extraUSDSAmount: spDeposit, extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(218, 16)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    //Bob provides USDS to SP
    ...
    // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated BNB
    ...
    assert.isAtMost(th.getDifference(bob_BNBGain_Before, liquidatedColl), 1000)
  ...
  // read StabilityPool and confirm
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and BNB gain", async () => {
    const A_spDeposit = toBN(dec(3, 24))
    const B_spDeposit = toBN(dec(1, 24))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    await openTrove({ ICR: toBN(dec(8, 18)), extraUSDSAmount: A_spDeposit, extraParams: { from: alice } })
    const { collateral: B_collateral, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(218, 16)), extraUSDSAmount: B_spDeposit, extraParams: { from: bob } })
    const { collateral: C_collateral, totalDebt: C_debt } = await openTrove({ ICR: toBN(dec(210, 16)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    assert.isAtMost(th.getDifference(bob_BNBGain_Before, th.applyLiquidationFee(C_collateral)), 1000)

    // Alice provides USDS to SP
    ...
    /* Alice's USDS Loss = (300 / 400) * 200 = 150 USDS
       Alice's BNB gain = (300 / 400) * 2*0.995 = 1.4925 BNB

       Bob's USDSLoss = (100 / 400) * 200 = 50 USDS
       Bob's BNB gain = (100 / 400) * 2*0.995 = 0.4975 BNB

     Check Bob' SP deposit has been reduced to 50 USDS, and his BNB gain has increased to 1.5 BNB. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedUSDSDeposit(alice)).toString()
    ...
    assert.isAtMost(th.getDifference(alice_BNBGain_After, th.applyLiquidationFee(B_collateral).mul(A_spDeposit).div(totalDeposits)), 3000000)

    const bob_Deposit_After = await stabilityPool.getCompoundedUSDSDeposit(bob)
    const bob_BNBGain_After = await stabilityPool.getDepositorBNBGain(bob)
    ...
    assert.isAtMost(th.getDifference(bob_BNBGain_After, bob_BNBGain_Before.add(th.applyLiquidationFee(B_collateral).mul(bob_Deposit_Before).div(totalDeposits))), 1000000)
  })

  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    const { usdsAmount: A_usdsAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: toBN(dec(300, 18)), extraParams: { from: alice } })
    const { usdsAmount: B_usdsAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: toBN(dec(200, 18)), extraParams: { from: bob } })
    const { usdsAmount: C_usdsAmount } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: carol } })
    ...
    // Liquidate A, B and C
    const activeUSDSDebt_0 = await activePool.getUSDSDebt()
    const defaultUSDSDebt_0 = await defaultPool.getUSDSDebt()

    await troveManager.liquidate(alice, DEFAULT_PRICE_FEED_DATA)
    const activeUSDSDebt_A = await activePool.getUSDSDebt()
    const defaultUSDSDebt_A = await defaultPool.getUSDSDebt()

    await troveManager.liquidate(bob, DEFAULT_PRICE_FEED_DATA)
    const activeUSDSDebt_B = await activePool.getUSDSDebt()
    const defaultUSDSDebt_B = await defaultPool.getUSDSDebt()
    ...
    assert.equal((await usdsToken.balanceOf(alice)).toString(), A_usdsAmount)
    assert.equal((await usdsToken.balanceOf(bob)).toString(), B_usdsAmount)
    assert.equal((await usdsToken.balanceOf(carol)).toString(), C_usdsAmount)
  })

  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    await openTrove({ ICR: toBN(dec(8, 18)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(221, 16)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: carol } })

    // Defaulter opens with 60 USDS, 0.6 BNB
    ...
    /* Liquidate defaulter. 30 USDS and 0.3 BNB is distributed between A, B and C.

    A receives (30 * 2/4) = 15 USDS, and (0.3*2/4) = 0.15 BNB
    B receives (30 * 1/4) = 7.5 USDS, and (0.3*1/4) = 0.075 BNB
    C receives (30 * 1/4) = 7.5 USDS, and (0.3*1/4) = 0.075 BNB
    */
    await troveManager.liquidate(defaulter_1, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidate(): when SP > 0, triggers SABLE reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalUSDSDeposits(), dec(100, 18))

    // const G_Before = await stabilityPool.epochToScaleToG(0, 0)

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)

    // Price drops to 1BNB:100USDS, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    assert.isFalse(await th.checkRecoveryMode(contracts))
  ...
  it('liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    const liqTxA = await troveManager.liquidate(A, DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(liqTxA.receipt.status)
    assert.isFalse(await sortedTroves.contains(A))

    // A adds 10 USDS to the SP, but less than C's debt
    ...
    const liqTxBC = await troveManager.liquidateTroves(2, DEFAULT_PRICE_FEED_DATA)
    ...
    await borrowerOperations.repayUSDS(dec(1, 18), D, D, DEFAULT_PRICE_FEED_DATA, {from: D})
    await borrowerOperations.repayUSDS(dec(1, 18), E, E, DEFAULT_PRICE_FEED_DATA, {from: E})
    ...
    const pendingBNB_C = await troveManager.getPendingBNBReward(C)
    const pendingUSDSDebt_C = await troveManager.getPendingUSDSDebtReward(C)
    const defaultPoolBNB = await defaultPool.getBNB()
    const defaultPoolUSDSDebt = await defaultPool.getUSDSDebt()
    assert.isTrue(pendingBNB_C.lte(defaultPoolBNB))
    assert.isTrue(pendingUSDSDebt_C.lte(defaultPoolUSDSDebt))
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingBNB_C, defaultPoolBNB), 1000)
    assert.isAtMost(th.getDifference(pendingUSDSDebt_C, defaultPoolUSDSDebt), 1000)
    ...
    const liqTx2 = await troveManager.liquidateTroves(2, DEFAULT_PRICE_FEED_DATA)
  ...
  it('liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves', async () => {
    ...
    // Price drops to 1BNB:100USDS, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    ...
    // Liquidate 5 troves
    await troveManager.liquidateTroves(5, DEFAULT_PRICE_FEED_DATA);
  ...
  it('liquidateTroves(): liquidates  up to the requested number of undercollateralized troves', async () => {
    ...
    await troveManager.liquidateTroves(3, DEFAULT_PRICE_FEED_DATA)
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110%', async () => {
    ...
    await assertInvalidOpcode(troveManager.liquidateTroves(10, DEFAULT_PRICE_FEED_DATA))
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 USDS and 0.3 BNB is distributed uniformly between A, B and C. Each receive 10 USDS, 0.1 BNB
    await troveManager.liquidate(defaulter_1, DEFAULT_PRICE_FEED_DATA)
    ...
    // Whale enters system, pulling it into Normal Mode
    await openTrove({ ICR: toBN(dec(10, 18)), extraUSDSAmount: dec(1, 24), extraParams: { from: whale } })

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    //liquidate A, B, C
    await troveManager.liquidateTroves(10, DEFAULT_PRICE_FEED_DATA)
  ...
  it("liquidateTroves(): reverts if n = 0", async () => {
    ...
    await assertInvalidOpcode(troveManager.liquidateTroves(0, DEFAULT_PRICE_FEED_DATA))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await usdsToken.balanceOf(dennis)
    const E_balanceBefore = await usdsToken.balanceOf(erin)
    const F_balanceBefore = await usdsToken.balanceOf(flyn)
    ...
    assert.equal((await usdsToken.balanceOf(dennis)).toString(), D_balanceBefore)
    assert.equal((await usdsToken.balanceOf(erin)).toString(), E_balanceBefore)
    assert.equal((await usdsToken.balanceOf(flyn)).toString(), F_balanceBefore)
  })

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 USDS to SP
    await openTrove({ ICR: toBN(dec(100, 18)), extraUSDSAmount: toBN(dec(500, 18)), extraParams: { from: whale } })
    ...
    // Check pool has 500 USDS
    assert.equal((await stabilityPool.getTotalUSDSDeposits()).toString(), dec(500, 18))
    ...
    assert.equal((await stabilityPool.getTotalUSDSDeposits()).toString(), '0')
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and BNB gain", async () => {
    // Whale provides 400 USDS to the SP
    const whaleDeposit = toBN(dec(40000, 18))
    await openTrove({ ICR: toBN(dec(100, 18)), extraUSDSAmount: whaleDeposit, extraParams: { from: whale } })
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: A_deposit, extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: B_deposit, extraParams: { from: bob } })
    ...
    // Check 800 USDS in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit)
    assert.equal((await stabilityPool.getTotalUSDSDeposits()).toString(), totalDeposits)
    ...
    Total USDS in Pool: 800 USDS

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 USDS
    Total liquidated BNB = 1.1 + 3.1 + 1.1 = 5.3 BNB

    whale usds loss: 650 * (400/800) = 325 usds
    alice usds loss:  650 *(100/800) = 81.25 usds
    bob usds loss: 650 * (300/800) = 243.75 usds

    whale remaining deposit: (400 - 325) = 75 usds
    alice remaining deposit: (100 - 81.25) = 18.75 usds
    bob remaining deposit: (300 - 243.75) = 56.25 usds
    ...
    Total remaining deposits: 150 USDS
    Total BNB gain: 4.975 BNB */

    // Check remaining USDS Deposits and BNB gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedUSDSDeposit(whale)
    const alice_Deposit_After = await stabilityPool.getCompoundedUSDSDeposit(alice)
    const bob_Deposit_After = await stabilityPool.getCompoundedUSDSDeposit(bob)

    const whale_BNBGain = await stabilityPool.getDepositorBNBGain(whale)
    const alice_BNBGain = await stabilityPool.getDepositorBNBGain(alice)
    const bob_BNBGain = await stabilityPool.getDepositorBNBGain(bob)
    ...
    assert.isAtMost(th.getDifference(whale_BNBGain, th.applyLiquidationFee(liquidatedColl).mul(whaleDeposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(alice_BNBGain, th.applyLiquidationFee(liquidatedColl).mul(A_deposit).div(totalDeposits)), 100000)
    assert.isAtMost(th.getDifference(bob_BNBGain, th.applyLiquidationFee(liquidatedColl).mul(B_deposit).div(totalDeposits)), 100000)

    // Check total remaining deposits and BNB gain in Stability Pool
    const total_USDSinSP = (await stabilityPool.getTotalUSDSDeposits()).toString()
    const total_BNBinSP = (await stabilityPool.getBNB()).toString()

    assert.isAtMost(th.getDifference(total_USDSinSP, totalDeposits.sub(liquidatedDebt)), 1000)
    assert.isAtMost(th.getDifference(total_BNBinSP, th.applyLiquidationFee(liquidatedColl)), 1000)
  })

  it("liquidateTroves(): when SP > 0, triggers SABLE reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

    // A, B, C open troves
    await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraUSDSAmount: toBN(dec(100, 18)), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(3, 18)), extraParams: { from: C } })
    ...
    // Liquidate troves
    await troveManager.liquidateTroves(2, DEFAULT_PRICE_FEED_DATA)
    assert.isFalse(await sortedTroves.contains(defaulter_1))
    assert.isFalse(await sortedTroves.contains(defaulter_2))
  ...
  it('batchLiquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards', async () => {
    ...
    // Try to liquidate C again. Check it succeeds and closes C's trove
    const liqTx2 = await troveManager.batchLiquidateTroves([C,D], DEFAULT_PRICE_FEED_DATA)
    assert.isTrue(liqTx2.receipt.status)
    assert.isFalse(await sortedTroves.contains(C))
    assert.isFalse(await sortedTroves.contains(D))
    assert.isTrue(await sortedTroves.contains(E))
  ...
  it('batchLiquidateTroves(): closes every trove with ICR < MCR in the given array', async () => {
    ...
    // Price drops to 1BNB:100USDS, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice()
    ...
    await troveManager.batchLiquidateTroves(liquidationArray, DEFAULT_PRICE_FEED_DATA);
  ...
  it('batchLiquidateTroves(): does not liquidate troves that are not in the given array', async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: toBN(dec(500, 18)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: toBN(dec(500, 18)), extraParams: { from: erin } })
  ...
  it('batchLiquidateTroves(): reverts if array is empty', async () => {
    ...
    liquidationArray = []
    try {
      const tx = await troveManager.batchLiquidateTroves(liquidationArray, DEFAULT_PRICE_FEED_DATA);
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraUSDSAmount: spDeposit, extraParams: { from: whale } })
    ...
    th.assertIsApproximatelyEqual((await stabilityPool.getTotalUSDSDeposits()).toString(), spDeposit.sub(A_debt).sub(B_debt))
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await usdsToken.transfer(carol, dec(100, 18), { from: whale })
    ...
    // Carol liquidated, and her trove is closed
    const txCarolClose = await borrowerOperations.closeTrove(DEFAULT_PRICE_FEED_DATA, { from: carol })
  ...
  it("batchLiquidateTroves: when SP > 0, triggers SABLE reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } })
    ...
    // Liquidate troves
    await troveManager.batchLiquidateTroves([defaulter_1, defaulter_2], DEFAULT_PRICE_FEED_DATA)
    assert.isFalse(await sortedTroves.contains(defaulter_1))
    assert.isFalse(await sortedTroves.contains(defaulter_2))
  ...
  it('getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption', async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraUSDSAmount: partialRedemptionAmount, extraParams: { from: alice } })
  ...
  it('getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations', async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra USDS. At least 3 iterations are needed
  ...
  it('redeemCollateral(): cancels the provided USDS with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(310, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(290, 16)), extraUSDSAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: carol } })
    const partialRedemptionAmount = toBN(2)
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount)
    // start Dennis with a high ICR
    await openTrove({ ICR: toBN(dec(100, 18)), extraUSDSAmount: redemptionAmount, extraParams: { from: dennis } })

    const dennis_BNBBalance_Before = toBN(await web3.eth.getBalance(dennis))

    const dennis_USDSBalance_Before = await usdsToken.balanceOf(dennis)
    ...
    // Find hints for redeeming 20 USDS
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(redemptionAmount, price, 0)
    ...
    const BNBFee = th.getEmittedRedemptionValues(redemptionTx)[3]
    ...
    /* check that Dennis' redeemed 20 USDS has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) USDS debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount))
    assert.equal(bob_debt_After, '0')
    assert.equal(carol_debt_After, '0')

    const dennis_BNBBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedBNB = dennis_BNBBalance_After.sub(dennis_BNBBalance_Before)

    const expectedTotalBNBDrawn = redemptionAmount.div(toBN(200)) // convert redemptionAmount USDS to BNB, at BNB:USD price 200
    const expectedReceivedBNB = expectedTotalBNBDrawn.sub(toBN(BNBFee)).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received BNB
    
    // console.log("*********************************************************************************")
    // console.log("BNBFee: " + BNBFee)
    // console.log("dennis_BNBBalance_Before: " + dennis_BNBBalance_Before)
    // console.log("GAS_USED: " + th.gasUsed(redemptionTx))
    // console.log("dennis_BNBBalance_After: " + dennis_BNBBalance_After)
    // console.log("expectedTotalBNBDrawn: " + expectedTotalBNBDrawn)
    // console.log("recived  : " + receivedBNB)
    // console.log("expected : " + expectedReceivedBNB)
    // console.log("wanted :   " + expectedReceivedBNB.sub(toBN(GAS_PRICE)))
    // console.log("*********************************************************************************")
    th.assertIsApproximatelyEqual(expectedReceivedBNB, receivedBNB)

    const dennis_USDSBalance_After = (await usdsToken.balanceOf(dennis)).toString()
    assert.equal(dennis_USDSBalance_After, dennis_USDSBalance_Before.sub(redemptionAmount))
  })
  ...
  it('redeemCollateral(): ends the redemption sequence when the token redemption request has been filled', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt } = await openTrove({ ICR: toBN(dec(290, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: carol } })
    const redemptionAmount = A_debt.add(B_debt).add(C_debt)
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: dennis } })
    const { totalDebt: E_totalDebt, collateral: E_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: erin } })

    // --- TEST --- 

    // open trove from redeemer.  Redeemer has highest ICR (100BNB, 100 USDS), 20000%
    const { usdsAmount: F_usdsAmount } = await openTrove({ ICR: toBN(dec(200, 18)), extraUSDSAmount: redemptionAmount.mul(toBN(2)), extraParams: { from: flyn } })

    // skip bootstrapping phase
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_WEEK * 2, web3.currentProvider)

    // Flyn redeems collateral
    await troveManager.redeemCollateral(redemptionAmount, alice, alice, alice, 0, 0, th._100pct, DEFAULT_PRICE_FEED_DATA, { from: flyn })

    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 USDS
    const flynBalance = await usdsToken.balanceOf(flyn)
    th.assertIsApproximatelyEqual(flynBalance, F_usdsAmount.sub(redemptionAmount))
  ...
  it('redeemCollateral(): ends the redemption sequence when max iterations have been reached', async () => {
    ...
    const { netDebt: A_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: alice } })
    const { netDebt: B_debt } = await openTrove({ ICR: toBN(dec(286, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: bob } })
    const { netDebt: C_debt, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(286, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: carol } })
    ...
    await troveManager.redeemCollateral(attemptedRedemptionAmount, alice, alice, alice, 0, 2, th._100pct, DEFAULT_PRICE_FEED_DATA, { from: flyn })

    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 USDS
    const flynBalance = (await usdsToken.balanceOf(flyn)).toString()
    th.assertIsApproximatelyEqual(flynBalance, F_usdsAmount.sub(redemptionAmount))
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(10000, 18)), A, A, DEFAULT_PRICE_FEED_DATA, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(20000, 18)), B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(30000, 18)), C, C, DEFAULT_PRICE_FEED_DATA, { from: C, value: dec(1000, 'ether') })

    // A and C send all their tokens to B
    await usdsToken.transfer(B, await usdsToken.balanceOf(A), {from: A})
    await usdsToken.transfer(B, await usdsToken.balanceOf(C), {from: C})
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(6000, 18)), A, A, DEFAULT_PRICE_FEED_DATA, { from: A, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(20000, 18)), B, B, DEFAULT_PRICE_FEED_DATA, { from: B, value: dec(1000, 'ether') })
    await borrowerOperations.openTrove(th._100pct, await getOpenTroveUSDSAmount(dec(30000, 18)), C, C, DEFAULT_PRICE_FEED_DATA, { from: C, value: dec(1000, 'ether') })
    ...
    // A's remaining debt would be 29950 + 19950 + 5950 + 50 - 54500 = 1400.
  ...
  it('redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date', async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(363, 16)), extraUSDSAmount: dec(5, 18), extraParams: { from: alice } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(344, 16)), extraUSDSAmount: dec(8, 18), extraParams: { from: bob } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(333, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: carol } })
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 USDS from Carol's Trove
    ...
    // Dennis tries to redeem 20 USDS
    const redemptionTx = await troveManager.redeemCollateral(
      redemptionAmount,
      firstRedemptionHint,
      upperPartialRedemptionHint,
    ...
    // Since Alice already redeemed 1 USDS from Carol's Trove, Dennis was  able to redeem:
    //  - 9 USDS from Carol's
    //  - 8 USDS from Bob's
    // for a total of 17 USDS.

    // Dennis calculated his hint for redeeming 2 USDS from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 USDS to fully complete his redemption of 20 USDS.
    // This would have required a different hint, therefore he ended up with a partial redemption.

    const dennis_BNBBalance_After = toBN(await web3.eth.getBalance(dennis))
    const receivedBNB = dennis_BNBBalance_After.sub(dennis_BNBBalance_Before)

    // Expect only 17 worth of BNB drawn
    const expectedTotalBNBDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)) // redempted USDS converted to BNB, at BNB:USD price 200
    const expectedReceivedBNB = expectedTotalBNBDrawn.sub(BNBFee).sub(toBN(th.gasUsed(redemptionTx) * GAS_PRICE)) // substract gas used for troveManager.redeemCollateral from expected received BNB

    th.assertIsApproximatelyEqual(expectedReceivedBNB, receivedBNB)

    const dennis_USDSBalance_After = (await usdsToken.balanceOf(dennis)).toString()
    th.assertIsApproximatelyEqual(dennis_USDSBalance_After, dennis_USDSBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion)))
  })
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveUSDSAmount(dec(110, 18))
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(133, 16)), extraUSDSAmount: amount, extraParams: { from: bob } })

    await usdsToken.transfer(carol, amount, { from: bob })

    const price = dec(100, 18)
    await priceFeed.setPrice(price)

    // Liquidate Bob's Trove
    await troveManager.liquidateTroves(1, DEFAULT_PRICE_FEED_DATA)

    // --- TEST --- 

    const carol_BNBBalance_Before = toBN(await web3.eth.getBalance(carol))
    ...
    const carol_BNBBalance_After = toBN(await web3.eth.getBalance(carol))

    const expectedTotalBNBDrawn = toBN(amount).div(toBN(100)) // convert 100 USDS to BNB at BNB:USD price of 100
    const expectedReceivedBNB = expectedTotalBNBDrawn.sub(BNBFee)

    const receivedBNB = carol_BNBBalance_After.sub(carol_BNBBalance_Before)
    assert.isTrue(expectedReceivedBNB.eq(receivedBNB))

    const carol_USDSBalance_After = (await usdsToken.balanceOf(carol)).toString()
    assert.equal(carol_USDSBalance_After, '0')
  })
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { usdsAmount: B_usdsAmount, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(133, 16)), extraUSDSAmount: A_debt, extraParams: { from: bob } })

    await usdsToken.transfer(carol, B_usdsAmount, { from: bob })
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: amount1, extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: amount1, extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: amount1, extraParams: { from: carol } })
    const redemptionAmount = C_totalDebt.add(B_totalDebt).add(A_totalDebt)
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraUSDSAmount: redemptionAmount, extraParams: { from: dennis } })
    ...
    await openTrove({ ICR: toBN(dec(100, 18)), extraUSDSAmount: dec(15, 18), extraParams: { from: whale } })
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500USDS to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(500, 18), extraParams: { from: alice } })
    await usdsToken.transfer(erin, dec(500, 18), { from: alice })
    ...
    const redemptionTxPromise = troveManager.redeemCollateral(0, erin, erin, erin, 0, 0, th._100pct, DEFAULT_PRICE_FEED_DATA, { from: erin })
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(10, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(20, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(30, 18), extraParams: { from: C } })
    await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(40, 18), extraParams: { from: D } })
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(9500, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(395, 16)), extraUSDSAmount: dec(9000, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(390, 16)), extraUSDSAmount: dec(10000, 18), extraParams: { from: C } })
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt)
    ...
    // USDS redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedUSDSRedemption = expectedTotalSupply.div(toBN(10))
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or BNB gain of redeemed-from troves", async () => {
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // B, C, D, F open trove
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(195, 16)), extraUSDSAmount: dec(200, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(400, 18), extraParams: { from: dennis } })
    const { totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: flyn } })

    const redemptionAmount = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(F_totalDebt)
    // Alice opens trove and transfers USDS to Erin, the would-be redeemer
    await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: redemptionAmount, extraParams: { from: alice } })
    await usdsToken.transfer(erin, redemptionAmount, { from: alice })
    ...
    // Liquidate Flyn
    await troveManager.liquidate(flyn, DEFAULT_PRICE_FEED_DATA)
    assert.isFalse(await sortedTroves.contains(flyn))

    // Price bounces back, bringing B, C, D back above MCR
    await priceFeed.setPrice(dec(200, 18))

    const bob_SPDeposit_before = (await stabilityPool.getCompoundedUSDSDeposit(bob)).toString()
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedUSDSDeposit(carol)).toString()
    const dennis_SPDeposit_before = (await stabilityPool.getCompoundedUSDSDeposit(dennis)).toString()

    const bob_BNBGain_before = (await stabilityPool.getDepositorBNBGain(bob)).toString()
    const carol_BNBGain_before = (await stabilityPool.getDepositorBNBGain(carol)).toString()
    const dennis_BNBGain_before = (await stabilityPool.getDepositorBNBGain(dennis)).toString()

    // Check the remaining USDS and BNB in Stability Pool after liquidation is non-zero
    const USDSinSP = await stabilityPool.getTotalUSDSDeposits()
    const BNBinSP = await stabilityPool.getBNB()
    assert.isTrue(USDSinSP.gte(mv._zeroBN))
    assert.isTrue(BNBinSP.gte(mv._zeroBN))
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedUSDSDeposit(bob)).toString()
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedUSDSDeposit(carol)).toString()
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedUSDSDeposit(dennis)).toString()

    const bob_BNBGain_after = (await stabilityPool.getDepositorBNBGain(bob)).toString()
    const carol_BNBGain_after = (await stabilityPool.getDepositorBNBGain(carol)).toString()
    const dennis_BNBGain_after = (await stabilityPool.getDepositorBNBGain(dennis)).toString()

    // Check B, C, D Stability Pool deposits and BNB gain have not been affected by redemptions from their troves
  ...
  it("redeemCollateral(): caller can redeem their entire USDSToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 400 USDS to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(400, 18), extraParams: { from: alice } })
    await usdsToken.transfer(erin, dec(400, 18), { from: alice })

    // Check Erin's balance before
    const erin_balance_before = await usdsToken.balanceOf(erin)
    assert.equal(erin_balance_before, dec(400, 18))

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(590, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(1990, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(500, 16)), extraUSDSAmount: dec(1990, 18), extraParams: { from: dennis } })
    ...
    const activePool_coll_before = await activePool.getBNB()

    th.assertIsApproximatelyEqual(activePool_debt_before, totalDebt)
    assert.equal(activePool_coll_before.toString(), totalColl.toString())
    ...
    // Erin attempts to redeem 400 USDS
    const {
      firstRedemptionHint,
      partialRedemptionHintNICR
    ...
    // Check activePool debt reduced by  400 USDS
    const activePool_debt_after = await activePool.getUSDSDebt()
    assert.equal(activePool_debt_before.sub(activePool_debt_after), dec(400, 18))

    /* Check ActivePool coll reduced by $400 worth of Ether: at BNB:USD price of $200, this should be 2 BNB.

    therefore remaining ActivePool BNB should be 198 */
    const activePool_coll_after = await activePool.getBNB()
    ...
    const erin_balance_after = (await usdsToken.balanceOf(erin)).toString()
    assert.equal(erin_balance_after, '0')
  })

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's USDS token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })
    ...
    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getUSDSDebt()
    const activePool_coll_before = (await activePool.getBNB()).toString()
    ...
    // Erin tries to redeem 1000 USDS
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
      ...
      assert.include(error.message, "Requested redemption amount must be <= user's USDS token balance")
    }

    // Erin tries to redeem 401 USDS
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 239482309 USDS
    try {
      ({
        firstRedemptionHint,
        partialRedemptionHintNICR
    ...
    // Erin tries to redeem 2^256 - 1 USDS
  ...
  it("redeemCollateral(): value of issued BNB == face value of redeemed USDS (assuming 1 USDS has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: whale } })

    // Alice opens trove and transfers 1000 USDS each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(400, 16)), extraUSDSAmount: dec(4990, 18), extraParams: { from: alice } })
    await usdsToken.transfer(erin, dec(1000, 18), { from: alice })
    await usdsToken.transfer(flyn, dec(1000, 18), { from: alice })
    await usdsToken.transfer(graham, dec(1000, 18), { from: alice })

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(300, 16)), extraUSDSAmount: dec(1590, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(600, 16)), extraUSDSAmount: dec(1090, 18), extraParams: { from: carol } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(800, 16)), extraUSDSAmount: dec(1090, 18), extraParams: { from: dennis } })

    const totalColl = W_coll.add(A_coll).add(B_coll).add(C_coll).add(D_coll)

    const price = await priceFeed.getPrice()

    const _120_USDS = '120000000000000000000'
    const _373_USDS = '373000000000000000000'
    const _950_USDS = '950000000000000000000'

    // Check Ether in activePool
    const activeBNB_0 = await activePool.getBNB()
    assert.equal(activeBNB_0.toString(), totalColl.toString());

    let firstRedemptionHint
    let partialRedemptionHintNICR


    // Erin redeems 120 USDS
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_120_USDS, price, 0))
    ...
    /* 120 USDS redeemed.  Expect $120 worth of BNB removed. At BNB:USD price of $200, 
    BNB removed = (120/200) = 0.6 BNB
    Total active BNB = 280 - 0.6 = 279.4 BNB */

    const activeBNB_1 = await activePool.getBNB()
    assert.equal(activeBNB_1.toString(), activeBNB_0.sub(toBN(_120_USDS).mul(mv._1e18BN).div(price)));

    // Flyn redeems 373 USDS
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_373_USDS, price, 0))
    ...
    /* 373 USDS redeemed.  Expect $373 worth of BNB removed. At BNB:USD price of $200, 
    BNB removed = (373/200) = 1.865 BNB
    Total active BNB = 279.4 - 1.865 = 277.535 BNB */
    const activeBNB_2 = await activePool.getBNB()
    assert.equal(activeBNB_2.toString(), activeBNB_1.sub(toBN(_373_USDS).mul(mv._1e18BN).div(price)));

    // Graham redeems 950 USDS
    ({
      firstRedemptionHint,
      partialRedemptionHintNICR
    } = await hintHelpers.getRedemptionHints(_950_USDS, price, 0))
    ...
    /* 950 USDS redeemed.  Expect $950 worth of BNB removed. At BNB:USD price of $200, 
    BNB removed = (950/200) = 4.75 BNB
    Total active BNB = 277.535 - 4.75 = 272.785 BNB */
    const activeBNB_3 = (await activePool.getBNB()).toString()
    assert.equal(activeBNB_3.toString(), activeBNB_2.sub(toBN(_950_USDS).mul(mv._1e18BN).div(price)));
  })
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDS to Bob
    await usdsToken.unprotectedMint(bob, dec(100, 18))

    assert.equal((await usdsToken.balanceOf(bob)), dec(100, 18))
    ...
    // Bob tries to redeem his illegally obtained USDS
    try {
      const redemptionTx = await troveManager.redeemCollateral(
        dec(100, 18),
        firstRedemptionHint,
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDS to Bob
    await usdsToken.unprotectedMint(bob, '101000000000000000000')

    assert.equal((await usdsToken.balanceOf(bob)), '101000000000000000000')

    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraUSDSAmount: dec(40, 18), extraParams: { from: carol } })
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(1000, 16)), extraUSDSAmount: dec(40, 18), extraParams: { from: dennis } })

    const totalDebt = C_totalDebt.add(D_totalDebt)
    th.assertIsApproximatelyEqual((await activePool.getUSDSDebt()).toString(), totalDebt)
    ...
    // Bob attempts to redeem his ill-gotten 101 USDS, from a system that has 100 USDS outstanding debt
    try {
      const redemptionTx = await troveManager.redeemCollateral(
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: A } })
    await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })
    ...
    const A_balanceBefore = await usdsToken.balanceOf(A)

    await th.redeemCollateral(A, contracts, dec(10, 18), GAS_PRICE)

    // Check A's balance has decreased by 10 USDS
    assert.equal(await usdsToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await usdsToken.balanceOf(A)
    const B_balanceBefore = await usdsToken.balanceOf(B)
    ...
    // Check B's balance has decreased by 10 USDS
    assert.equal(await usdsToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString())
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 USDS
    assert.equal(A_balanceBefore.sub(await usdsToken.balanceOf(A)), dec(10, 18))
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero BNBFee to SABLE staking contract", async () => {
    ...
    // Check SABLE Staking contract balance before is zero
    const sableStakingBalance_Before = await web3.eth.getBalance(sableStaking.address)
    assert.equal(sableStakingBalance_Before, '0')
    ...
    // Check SABLE Staking contract balance after is non-zero
    const sableStakingBalance_After = toBN(await web3.eth.getBalance(sableStaking.address))
    assert.isTrue(sableStakingBalance_After.gt(toBN('0')))
  })
  ...
  it("redeemCollateral(): a redemption made at zero base increases the BNB-fees-per-SABLE-staked in SABLE Staking contract", async () => {
    ...
    // Check SABLE Staking BNB-fees-per-SABLE-staked before is zero
    ...
    // Check SABLE Staking BNB-fees-per-SABLE-staked after is non-zero
    const F_BNB_After = await sableStaking.F_BNB()
    assert.isTrue(F_BNB_After.gt('0'))
  })
  ...
  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero BNBFee to SABLE staking contract", async () => {
    ...
    const sableStakingBalance_Before = toBN(await web3.eth.getBalance(sableStaking.address))
    ...
    // check SABLE Staking balance has increased
    assert.isTrue(sableStakingBalance_After.gt(sableStakingBalance_Before))
  })
  ...
  it("redeemCollateral(): a redemption made at a non-zero base rate increases BNB-per-SABLE-staked in the staking contract", async () => {
    ...
    // check SABLE Staking balance has increased
    assert.isTrue(F_BNB_After.gt(F_BNB_Before))
  })
  ...
  it("redeemCollateral(): a redemption sends the BNB remainder (BNBDrawn - BNBFee) to the redeemer", async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: A } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })
    ...
    // Check total USDS supply
    const activeUSDS = await activePool.getUSDSDebt()
    const defaultUSDS = await defaultPool.getUSDSDebt()

    const totalUSDSSupply = activeUSDS.add(defaultUSDS)
    th.assertIsApproximatelyEqual(totalUSDSSupply, totalDebt)
    ...
    /*
    At BNB:USD price of 200:
    BNBDrawn = (9 / 200) = 0.045 BNB
    BNBfee = (0.005 + 0.0025 + (1/2) *( 9/260)) * BNBDrawn = 0.0011163461538461537 BNB
    BNBRemainder = 0.045 - 0.0011163461538461537... = 0.04388365384615384
    */

    const A_balanceAfter = toBN(await web3.eth.getBalance(A))

    // check A's BNB balance has increased by 0.045 BNB 
    const price = await priceFeed.getPrice()
    const BNBDrawn = redemptionAmount.mul(mv._1e18BN).div(price)
    th.assertIsApproximatelyEqual(
      A_balanceAfter.sub(A_balanceBefore),
      BNBDrawn.sub(
        toBN(dec(75, 14)).add(redemptionAmount.mul(mv._1e18BN).div(totalDebt).div(toBN(2)))
          .mul(BNBDrawn).div(mv._1e18BN)
      ).sub(toBN(gasUsed * GAS_PRICE)), // substract gas used for troveManager.redeemCollateral from expected received BNB
      100000
    )
  })
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const { netDebt: A_netDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems 360 USDS.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const { netDebt: A_netDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: A } })
    const { netDebt: B_netDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt, collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })
    const { netDebt: D_netDebt } = await openTrove({ ICR: toBN(dec(280, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: D } })
    const redemptionAmount = A_netDebt.add(B_netDebt).add(C_netDebt).add(toBN(dec(10, 18)))
    ...
    // whale redeems USDS.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount, GAS_PRICE)
    ...
    /*
    At BNB:USD price of 200, with full redemptions from A, B, C:

    BNBDrawn from A = 100/200 = 0.5 BNB --> Surplus = (1-0.5) = 0.5
    BNBDrawn from B = 120/200 = 0.6 BNB --> Surplus = (1-0.6) = 0.4
    BNBDrawn from C = 130/200 = 0.65 BNB --> Surplus = (2-0.65) = 1.35
    */
    ...
    await assertRevert(borrowerOperations.openTrove(th._100pct, 0, ZERO_ADDRESS, ZERO_ADDRESS, DEFAULT_PRICE_FEED_DATA, { from: D, value: dec(10, 18) }), 'BorrowerOps: Trove is active')
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    const { netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    const { netDebt: C_netDebt } = await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })
    const { totalDebt: D_totalDebt, collateral: D_coll } = await openTrove({ ICR: toBN(dec(280, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: D } })
    ...
    // whale redeems USDS.  Expect this to fully redeem A, B, C, and partially redeem 15 USDS from D.
    ...
    /* Expect D to have lost 15 debt and (at BNB price of 200) 15/200 = 0.075 BNB. 
    So, expect remaining debt = (85 - 15) = 70, and remaining BNB = 1 - 15/200 = 0.925 remaining. */
    const price = await priceFeed.getPrice()
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's BNB surplus (collateral - BNB drawn) available for the trove owner to claim", async () => {
    const {
      A_netDebt, A_coll,
      B_netDebt, B_coll,
      C_netDebt, C_coll,
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's BNB surplus (collateral - BNB drawn) available for the trove owner after re-opening trove", async () => {
    const {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(190, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(180, 16)), extraUSDSAmount: dec(100, 18), extraParams: { from: C } })

    const A_collAfter = await troveManager.getTroveColl(A)
    const B_collAfter = await troveManager.getTroveColl(B)
    const C_collAfter = await troveManager.getTroveColl(C)

    assert.equal(Number(A_collAfter), Number(A_coll))
    assert.equal(Number(B_collAfter), Number(B_coll))
    assert.equal(Number(C_collAfter), Number(C_coll))
  ...
  it('redeemCollateral(): reverts if fee eats up all returned collateral', async () => {
    // --- SETUP ---
    const { usdsAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraUSDSAmount: dec(1, 24), extraParams: { from: alice } })
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const {
        firstRedemptionHint,
        partialRedemptionHintNICR
      } = await hintHelpers.getRedemptionHints(usdsAmount, price, 0)
      ...
      await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
      const AdjustTroveParam = {
        collWithdrawal: 0,
        USDSChange: usdsAmount.toString(),
        isDebtIncrease: true,
        upperHint: alice,
        lowerHint: alice,
        maxFeePercentage: th._100pct
      }
      // await borrowerOperations.adjustTrove(th._100pct, 0, usdsAmount, true, alice, alice, { from: alice, value: usdsAmount.mul(mv._1e18BN).div(price) })
      await borrowerOperations.adjustTrove(AdjustTroveParam, DEFAULT_PRICE_FEED_DATA, { from: alice, value: usdsAmount.mul(mv._1e18BN).div(price) })
    }
  ...
  it("getPendingUSDSDebtReward(): Returns 0 if there is no pending USDSDebt reward", async () => {
    // Make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: dec(100, 18), extraParams: { from: defaulter_1 } })

    await openTrove({ ICR: toBN(dec(3, 18)), extraUSDSAmount: dec(20, 18), extraParams: { from: carol } })

    await openTrove({ ICR: toBN(dec(20, 18)), extraUSDSAmount: totalDebt, extraParams: { from: whale } })
    ...
    const current_L_USDSDebt = await troveManager.L_USDSDebt()
    assert.equal(current_L_USDSDebt, 0)

    const carolSnapshot_L_USDSDebt = (await troveManager.rewardSnapshots(carol))[1]
    assert.equal(carolSnapshot_L_USDSDebt, 0)

    const carol_PendingUSDSDebtReward = await troveManager.getPendingUSDSDebtReward(carol)
    assert.equal(carol_PendingUSDSDebtReward, 0)
  })

  it("getPendingBNBReward(): Returns 0 if there is no pending BNB reward", async () => {
    // make some troves
    const { totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraUSDSAmount: dec(100, 18), extraParams: { from: defaulter_1 } })
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_BNB = await troveManager.L_BNB()
    assert.equal(current_L_BNB, 0)

    const carolSnapshot_L_BNB = (await troveManager.rewardSnapshots(carol))[0]
    assert.equal(carolSnapshot_L_BNB, 0)

    const carol_PendingBNBReward = await troveManager.getPendingBNBReward(carol)
    assert.equal(carol_PendingBNBReward, 0)
  })
  ...
  it("computeICR(): Returns 2^256-1 for BNB:USD = 100, coll = 1 BNB, debt = 100 USDS", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
    const debt = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for BNB:USD = 100, coll = 200 BNB, debt = 30 USDS", async () => {
    const price = dec(100, 18)
  ...
  it("computeICR(): returns correct ICR for BNB:USD = 250, coll = 1350 BNB, debt = 127 USDS", async () => {
  ...
  it("computeICR(): returns correct ICR for BNB:USD = 100, coll = 1 BNB, debt = 54321 USDS", async () => {
    const price = dec(100, 18)
    const coll = dec(1, 'ether')
  ...
  it("checkRecoveryMode(): Returns true when TCR < 150%", async () => {
    ...
    await priceFeed.setPrice(toBN('99999999999999999999'))
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({ ICR: toBN(dec(150, 16)), extraUSDSAmount: B_totalDebt, extraParams: { from: A } })

    // to be able to repay:
    await usdsToken.transfer(B, B_totalDebt, { from: A })
    await borrowerOperations.closeTrove(DEFAULT_PRICE_FEED_DATA, {from: B})
