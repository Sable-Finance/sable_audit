...
contract('HintHelpers', async accounts => {
  ...
  /* Open a Trove for each account. USDS debt is 200 USDS each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

 // Open Troves in parallel, then withdraw USDS in parallel
 const makeTrovesInParallel = async (accounts, n) => {
  ...
  const withdrawUSDSpromises = activeAccounts.map(account => withdrawUSDSfromTrove(account))
  await Promise.all(withdrawUSDSpromises)
 ...
 const withdrawUSDSfromTrove = async (account) => {
  await borrowerOperations.withdrawUSDS(th._100pct, '100000000000000000000', account, account, { from: account })
 }

 // Sequentially add coll and withdraw USDS, 1 account at a time
  const makeTrovesInSequence = async (accounts, n) => {
    ...
    for (const account of activeAccounts) {
      ...
      await th.openTrove(contracts, { extraUSDSAmount: toBN(dec(10000, 18)), ICR: ICR_BN, extraParams: { from: account } })
