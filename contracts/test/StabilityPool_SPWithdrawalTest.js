...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      const txB = await stabilityPool.withdrawFromSP(dec(20000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      const txC = await stabilityPool.withdrawFromSP(dec(30000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      */
      ...
      // Depositors attempt to withdraw everything
      const txA = await stabilityPool.withdrawFromSP(dec(500000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      const txB = await stabilityPool.withdrawFromSP(dec(500000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      const txC = await stabilityPool.withdrawFromSP(dec(500000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 LUSD.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      const txD = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      ...
      const txA = await stabilityPool.withdrawFromSP(dec(100000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      const txB = await stabilityPool.withdrawFromSP(dec(100000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
      const txC = await stabilityPool.withdrawFromSP(dec(100000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
      const txD = await stabilityPool.withdrawFromSP(dec(100000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct LUSD deposit and ETH Gain", async () => {
      ...
      */
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txD = await stabilityPool.withdrawFromSP(dec(40000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
      await priceFeed.setPrice(dec(100, 18));
    ...
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const txD = await stabilityPool.withdrawFromSP(dec(20000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
      const txE = await stabilityPool.withdrawFromSP(dec(30000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: erin
      });
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const txE = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: erin
      });
      const txF = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: flyn
      });
      const txG = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: graham
      });
      const txH = await stabilityPool.withdrawFromSP(dec(10000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: harriet
      });
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      const txC = await stabilityPool.withdrawFromSP(dec(20000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: carol
      });
      const txD = await stabilityPool.withdrawFromSP(dec(30000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: dennis
      });
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txA = await stabilityPool.withdrawFromSP(dec(100, 18), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      await priceFeed.setPrice(dec(100, 18));
    ...
    */
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txA = await stabilityPool.withdrawFromSP(dec(1000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: A
      });
      const txB = await stabilityPool.withdrawFromSP(dec(1000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: B
      });
      await priceFeed.setPrice(dec(100, 18));
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txC = await stabilityPool.withdrawFromSP(dec(1000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: C
      });
      const txD = await stabilityPool.withdrawFromSP(dec(1000, 18), DEFAULT_PRICE_FEED_DATA, {
        from: D
      });
      await priceFeed.setPrice(dec(100, 18));
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      const txA = await stabilityPool.withdrawFromSP(dec(1, 36), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      const txB = await stabilityPool.withdrawFromSP(dec(1, 36), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      const txA = await stabilityPool.withdrawFromSP(dec(1, 38), DEFAULT_PRICE_FEED_DATA, {
        from: alice
      });
      const txB = await stabilityPool.withdrawFromSP(dec(1, 38), DEFAULT_PRICE_FEED_DATA, {
        from: bob
      });
