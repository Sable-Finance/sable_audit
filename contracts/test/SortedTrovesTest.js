...
contract('SortedTroves', async accounts => {
  ...
  const [
    ...
    A, B, C, D, E, F, G, H, I, J, whale, funder] = accounts;
  ...
  describe('SortedTroves', () => {
    ...
    it('contains(): returns false for addresses that opened and then closed a trove', async () => {
      await openTrove({ ICR: toBN(dec(1000, 18)), extraUSDSAmount: toBN(dec(3000, 18)), extraParams: { from: whale } })
      ...
      await usdsToken.transfer(alice, dec(1000, 18), { from: whale })
      await usdsToken.transfer(bob, dec(1000, 18), { from: whale })
      await usdsToken.transfer(carol, dec(1000, 18), { from: whale })
      ...
      await borrowerOperations.closeTrove(DEFAULT_PRICE_FEED_DATA, { from: carol })
  ...
  describe('SortedTroves with mock dependencies', () => {
    ...
    context('when params are properly set', () => {
      ...
      it('insert(): fails if list is full', async () => {
        ...
        await sortedTrovesTester.insert(sortParam1)
        ...
        await sortedTrovesTester.insert(sortParam2)
        ...
        await th.assertRevert(sortedTrovesTester.insert(sortParam3), 'SortedTroves: List is full')
      })
      ...
      it('insert(): fails if list already contains the node', async () => {
        ...
        await th.assertRevert(sortedTrovesTester.insert(sortParam2), 'SortedTroves: List already contains the node')
      })
      ...
      it('insert(): fails if id is zero', async () => {
        ...
        await th.assertRevert(sortedTrovesTester.insert(sortParam1), 'SortedTroves: Id cannot be zero')
      })
      ...
      it('insert(): fails if NICR is zero', async () => {
        ...
        await th.assertRevert(sortedTrovesTester.insert(sortParam1), 'SortedTroves: NICR must be positive')
      })
      ...
      it('reInsert(): fails if list doesn’t contain the node', async () => {
        ...
        await th.assertRevert(sortedTrovesTester.reInsert(sortParam1), 'SortedTroves: List does not contain the id')
      })
      ...
      it('reInsert(): fails if new NICR is zero', async () => {
        ...
        await th.assertRevert(sortedTrovesTester.reInsert(sortParam2), 'SortedTroves: NICR must be positive')
        assert.isTrue(await sortedTroves.contains(alice), 'list should contain element')
      })
